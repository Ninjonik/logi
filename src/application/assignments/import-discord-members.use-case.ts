import {
    mergeImportedSecondaryGroupIds,
    validateAssignmentGroupIds,
} from "@/domain/assignments/policy"
...
export class ImportDiscordMembersUseCase {
    ...
    async execute(input: {
        ...
        const serverExists = await this.repository.serverExists(
        ...
        const touchedUserIds = [
            ...new Set(input.members.map((member) => member.userId)),
        ]
        await rebuildMembershipState(
            this.repository,
            input.serverDiscordId,
            touchedUserIds,
            now
