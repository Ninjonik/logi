...
>(
    ...
    return {
        ...
        rosters: filteredContexts.flatMap((context) =>
            context.roster ? [context.roster] : []
        ),
        syncStates: filteredContexts.flatMap((context) =>
            context.syncState ? [context.syncState] : []
        ),
    }
}
...
>(index: { events: TEvent[]; rosters: TRoster[] }) {
    const rosterIndexByEventId = new Map(
        index.rosters.map((roster) => [roster.eventId, roster])
    )
    ...
    for (const event of index.events) {
        ...
        signatures.set(
            event.id,
            `${event.updatedAt}|${roster?.updatedAt ?? "no-roster"}`
        )
    }
...
export function getChangedEventIds(
    ...
    for (const [eventId, signature] of nextSignaturesByEventId) {
        if (
            initialLoad ||
            previousSignaturesByEventId.get(eventId) !== signature
        ) {
