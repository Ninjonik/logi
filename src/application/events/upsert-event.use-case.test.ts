...
test("UpsertEventUseCase creates imported past matches as concluded", async () => {
    ...
    const useCase = new UpsertEventUseCase(
        repo,
        scores,
        new FakeClock(new Date("2026-07-25T12:00:00.000Z"))
    )
    ...
    assert.equal(
        repo.events.get(eventId)?.concludedAt,
...
test("UpsertEventUseCase updates an existing event and triggers scoring when concluded", async () => {
    const repo = new InMemoryEventCommandRepository(
        new Map([
            [
                "event-1",
                {
...
test("UpsertEventUseCase preserves creation-time Discord routing on update", async () => {
        ...
        new Map([
            [
                "event-1",
                {
                    ...
                    meetingStart: "2026-07-23T11:00:00.000Z",
                    gameEnd: "2026-07-23T14:00:00.000Z",
                    announcementChannelId: "registration",
                    eventInfoChannelId: "roster",
                    status: "registration",
                    participants: [],
                    signUps: [],
                    absenceNotices: [],
    ...
    await useCase.execute({
        ...
        gameStart: "2026-07-23T12:00:00.000Z",
        gameEnd: "2026-07-23T14:00:00.000Z",
        announcementChannelId: "other-registration",
        eventInfoChannelId: "other-roster",
        pingClan: false,
    })

    assert.equal(
        repo.events.get("event-1")?.announcementChannelId,
        "registration"
    )
