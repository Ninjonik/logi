...
import {
    ApplyEventScoreUseCase,
    type EventScoreRepository,
} from "./apply-event-score.use-case"
...
class InMemoryEventScoreRepository implements EventScoreRepository {
    ...
    async updateUserScore(
        userId: string,
        patch: { score: number; scores: Record<string, number> }
    ) {
    ...
    async markEventScoreApplied() {
        if (this.event)
            this.event = { ...this.event, scoreResolution: "applied" }
    }
    async markEventScoreSkipped() {
        if (this.event)
            this.event = { ...this.event, scoreResolution: "skipped" }
    }
}
...
test("ApplyEventScoreUseCase applies score deltas to active assignments only", async () => {
    const repo = new InMemoryEventScoreRepository(
        {
            ...
            participants: [
                ...
                { userId: "user-2", status: "not_attending" },
            ],
            absenceNotices: [],
    ...
    assert.equal(
        repo.users.find((user) => user.userId === "user-1")?.scores?.[
            "guild-1"
        ],
        15
    )
...
test("ApplyEventScoreUseCase rejects unknown events", async () => {
    const repo = new InMemoryEventScoreRepository(null, null, [], [], {
        noCategory: 0,
        declined: 1,
        rosterPresent: 5,
        reservePresent: 3,
    ...
    await assert.rejects(
        () => new ApplyEventScoreUseCase(repo).execute("missing"),
        /Event not found/
    )
})
...
test("ApplyEventScoreUseCase does nothing for unconcluded or already resolved events", async () => {
    ...
    assert.equal(
        await new ApplyEventScoreUseCase(unresolved).execute("event-1"),
        false
    )
    ...
    assert.equal(
        await new ApplyEventScoreUseCase(resolved).execute("event-1"),
        false
    )
