    ...
    return {
        ...
        game_layout: {
            requested: Array.isArray(payload.game_layout?.requested)
                ...
                : [],
            set: Array.isArray(payload.game_layout?.set)
...
function calculateImportPercent(
    progress: Pick<ImportProgress, "total" | "fetched" | "processed">
) {
    if (progress.total <= 0) {
        return 0
    }
        ...
        while (true) {
            ...
            try {
                ...
                input.onProgress?.({
                    ...
                    successful: 0,
                    failed: 0,
                    percent: calculateImportPercent({
                        total,
                        fetched,
                        processed: 0,
                    }),
...
function buildUserNicknameMap(users: AppUser[], serverDiscordId: string) {
    ...
    for (const user of users) {
        const normalizedNickname = normalizeComparableName(
            user.nicknames?.[serverDiscordId]
        )
...
function resolveImportedUserMatch(input: {
    ...
    const matchedByPlatformId = input.usersByPlatformId.get(
    ...
    const matchedByNickname = input.usersByNickname.get(
        input.normalizedPlayerName
    )
...
function resolveImportedPlayerName(
    playerName: string,
    ...
    return (
        stripClanTag(playerName, clanTag) ?? (playerName.trim() || playerName)
    )
}

function resolveLocalTeam(
    eventSide: string | undefined,
    payload: ScoreboardResponse["result"]
) {
    ...
    const axisNames = new Set(
        ["axis", payload.map.map.axis.team, payload.map.map.axis.name].map(
            normalizeValue
        )
    )
...
function buildEventResult(
    ...
    const sideA = resolvePreferredTeamLabel(
        [payload.map.map.axis.name, payload.map.map.axis.team],
        "Side A"
    )
    const sideB = resolvePreferredTeamLabel(
        [payload.map.map.allies.name, payload.map.map.allies.team],
        "Side B"
    )
...
async function buildServerUserLookups(
    serverId: string,
    importPlayers: boolean
): Promise<ImportUserLookupContext> {
    ...
    const assignedUsers = await getUsersByIds(
        assignments.map((assignment) => assignment.userId)
    )
    const assignedUserIds = new Set(assignedUsers.map((user) => user.id))
    const allUsers = importPlayers ? await listUsersUncached() : assignedUsers
    const nameMatchUsers = importPlayers
        ? allUsers.filter(
              (user) =>
                  assignedUserIds.has(user.id) ||
                  canImportExistingUserToServer(user, serverDiscordId)
          )
        : assignedUsers

    const usersByPlatformId = new Map(
        allUsers.flatMap((user) =>
            user.platformIds.map(
                (platformId) => [normalizeValue(platformId), user] as const
            )
        )
    )
    const usersByName = buildUserNameMap(nameMatchUsers)
    const usersByNickname = buildUserNicknameMap(
        nameMatchUsers,
        serverDiscordId
    )
...
function addUserToLookupMaps(context: ImportUserLookupContext, user: AppUser) {
    ...
    const normalizedNickname = normalizeComparableName(
        user.nicknames?.[context.serverDiscordId]
    )
    if (normalizedNickname) {
...
async function preparePlayerImports(input: {
    ...
    const lookupContext =
        input.lookupContext ??
        (await buildServerUserLookups(
            input.serverId,
            Boolean(input.importPlayers)
        ))
    const { usersByPlatformId, usersByName, usersByNickname, serverDiscordId } =
        lookupContext
    ...
    for (const player of input.payload.player_stats) {
        ...
        const strippedName = input.clanTag
            ? stripClanTag(player.player, input.clanTag)
            : null
        const importedPlayerName = resolveImportedPlayerName(
            player.player,
            input.clanTag
        )
        ...
        if (!matchedUser) {
            ...
            logNextInfo(
                "match-results:player-match",
                "Created imported player during event import",
                {
                    serverId: input.serverId,
            ...
            input.importPlayers &&
            normalizedPlayerId &&
            !matchedUser.platformIds.some(
                (platformId: string) =>
                    normalizeValue(platformId) === normalizedPlayerId
            )
        ) {
        ...
        if (matchReason === "platformId") {
            ...
            logNextInfo(
                "match-results:player-match",
                "Matched imported player by normalized name",
                {
                    serverId: input.serverId,
                    eventId: input.eventIdForLogs,
                    sourceUrl: input.sourceUrl,
            ...
            logNextInfo(
                "match-results:player-match",
                "Matched imported player by guild nickname",
                {
                    serverId: input.serverId,
                    eventId: input.eventIdForLogs,
                    sourceUrl: input.sourceUrl,
    ...
    const rankedSides = [...sideCounts.values()].sort(
        (left, right) => right.count - left.count
    )
    const inferredEventSide =
        rankedSides.length === 0
            ? undefined
            : rankedSides.length > 1 &&
                rankedSides[0].count === rankedSides[1].count
              ? undefined
              : rankedSides[0].value
...
function buildImportedEventInput(input: {
    ...
    return {
        ...
        server: Number.isFinite(payload.server_number)
            ? `Server ${payload.server_number}`
            : "-",
        serverPassword: "-",
...
export async function importEventMatchResults(input: {
    ...
    try {
        ...
        const sanitizedPayload = sanitizeScoreboardResult(
            fetched.payload.result
        )
        ...
        const resolvedEventSide =
            input.eventSide ?? preparedImport.inferredEventSide
        const eventResult = buildEventResult(
            resolvedEventSide,
            sourceUrl,
            mapId,
            sanitizedPayload
        ...
        if (eventResult) {
            ...
            diagnostics.eventResultSaved = {
                ok: false,
                error:
                    diagnostics.eventResultPrepared.skippedReason ??
                    "Event result was not prepared.",
            }
        }
...
export async function importServerEventsFromLinks(input: {
    ...
    const lookupContext = await buildServerUserLookups(
        input.serverId,
        Boolean(input.importPlayers)
    )
    ...
    for (const fetchedResult of fetchedResults) {
        ...
        const diagnostics: ImportDiagnostics = {
            scoreboardFetched: {
                ok: fetchedResult.ok,
                error: fetchedResult.ok ? undefined : fetchedResult.error,
            },
            playerStatsSaved: { ok: false },
            matchSaved: { ok: false },
            eventResultPrepared: { ok: false },
        ...
        try {
            ...
            importedPlayers += preparedImport.entries.length
            preparedImport.importedUserIds.forEach((userId) =>
                importedUserIds.add(userId)
            )
            logNextInfo(
                "match-results:event-import",
                "Imported match event from external scoreboard",
                {
                    serverId: input.serverId,
                    eventId,
                    sourceUrl,
                    ...
                    inferredEventSide,
                    matchedByPlatformId:
                        preparedImport.matchReasonCounts.platformId,
                    matchedByName: preparedImport.matchReasonCounts.name,
                    matchedByNickname:
                        preparedImport.matchReasonCounts.nickname,
                    createdImportedPlayers:
                        preparedImport.matchReasonCounts.createdImported,
                }
            )
            linkReports.push({
                link,
...
export async function autoLinkPlatformIdsFromEventImports(input: {
    ...
    const existingPlatformOwnerById = new Map(
        allUsers.flatMap((user) =>
            user.platformIds.map(
                (platformId) => [normalizeValue(platformId), user.id] as const
            )
        )
    )
    ...
    for (const sourceUrl of sourceUrls) {
        try {
            ...
            for (const player of payload.result.player_stats) {
                ...
                const matchedUser = uniqueUsersByName.get(
                    normalizeValue(strippedName)
                )
                if (!matchedUser) {
                    continue
                }
                ...
                const candidateIds =
                    candidateIdsByUserId.get(matchedUser.id) ??
                    new Set<string>()
    ...
    for (const user of assignedUsers) {
        ...
        const existingPlatformIds = user.platformIds
            .map((platformId) => normalizeValue(platformId))
            .filter(Boolean)
...
export async function linkMissingDiscordIdsFromRole(input: {
    ...
    const candidates = users.filter(
        (user) => !user.hasDiscordLink && serverUserIdSet.has(user.id)
    )
    ...
    for (const user of candidates) {
        const match = roleMembersByName.get(
            normalizeComparableName(user.name)
        )?.[0]
        if (!match) {
            continue
        }
