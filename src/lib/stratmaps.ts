...
export function getHllStratmapCatalogGroups() {
    return HLL_STRATMAP_CATALOG.reduce<
...
export function parseStratmapState(
    stateJson?: string | null,
    fallbackMapId?: string
) {
    if (!stateJson) {
        return buildDefaultStratmapState(
            fallbackMapId ?? HLL_STRATMAP_MAPS[0]?.id ?? "carentan"
        )
    }

    try {
        const parsed = JSON.parse(stateJson) as Omit<
            StratmapState,
            "slides"
        > & {
            slides?: Array<
                Omit<StratmapSlide, "overlays"> & {
                    overlays?: Partial<StratmapOverlaySettings> & {
                        offensiveGarrisonSide?: "a" | "b"
                    }
                }
            >
        }
        ...
        return {
            ...
            slides: parsed.slides.map((slide) => ({
                ...
                overlays: {
                    ...
                    showAllStrongpoints:
                        slide.overlays?.showAllStrongpoints ?? true,
                    visibleStrongpointIds:
                        slide.overlays?.visibleStrongpointIds ?? [],
                    showOffensiveGarrisons:
                        slide.overlays?.showOffensiveGarrisons ?? false,
                    overlayTeam:
                        slide.overlays?.overlayTeam ??
                        slide.overlays?.offensiveGarrisonSide ??
                        "a",
                    ...
                    element.kind === "icon"
                        ? {
                              ...
                              attachments: Array.isArray(element.attachments)
                                  ...
                                  : [],
                              mainAttachmentUrl:
                                  typeof element.mainAttachmentUrl === "string"
