...
const LOG_DB_PATH =
    process.env.LOGI_LOG_DB_PATH ?? path.join(process.cwd(), "data", "logs.db")
...
function getDatabaseSyncConstructor(): DatabaseSyncConstructor {
    ...
    return (
        requireNodeModule(moduleName) as {
...
export function writeSystemLog(input: {
    ...
    `
            ...
            typeof context?.requestPath === "string"
                ? context.requestPath
                : null
        )
    } catch (error) {
...
export function querySystemLogs(filters: SystemLogFilters) {
    ...
    if (filters.search?.trim()) {
        clauses.push(
            "(message LIKE ? OR scope LIKE ? OR IFNULL(context_json, '') LIKE ? OR IFNULL(server_id, '') LIKE ? OR IFNULL(user_id, '') LIKE ? OR IFNULL(request_path, '') LIKE ?)"
        )
    ...
    const db = ensureDatabase()
    const countRow = db
        .prepare(`SELECT COUNT(*) as total FROM system_logs ${whereClause}`)
...
export function getSystemLogStats() {
    const db = ensureDatabase()
    const totalRow = db
        .prepare("SELECT COUNT(*) as total FROM system_logs")
        .get() as { total: number }
    const errorRow = db
        .prepare(
            "SELECT COUNT(*) as total FROM system_logs WHERE level = 'ERROR'"
        )
        .get() as { total: number }
    const todayStart = new Date()
    todayStart.setHours(0, 0, 0, 0)
    const todayIso = todayStart.toISOString()
    const todayErrorRow = db
        .prepare(
            "SELECT COUNT(*) as total FROM system_logs WHERE level = 'ERROR' AND timestamp >= ?"
        )
        .get(todayIso) as { total: number }
    const nextRow = db
        .prepare(
            "SELECT COUNT(*) as total FROM system_logs WHERE source = 'nextjs'"
        )
        .get() as { total: number }
    const botRow = db
        .prepare(
            "SELECT COUNT(*) as total FROM system_logs WHERE source = 'discord-bot'"
        )
        .get() as { total: number }
    const scopes = db
        .prepare(
...
export async function getKnownLogScopes() {
    const db = ensureDatabase()
    const rows = db
        .prepare("SELECT DISTINCT scope FROM system_logs ORDER BY scope ASC")
