...
export function summarizeRosterUpdates(
    before: Roster,
    after: Roster,
    users: AppUser[]
): RosterUpdateSummary {
    ...
    for (const [userId, nextAssignment] of afterAssignments) {
        ...
        if (!previousAssignment) {
            ...
            addedLines.push(
                `🟢 ${userLabel} -> ${nextAssignment.squadName}${nextAssignment.roleName ? ` (${nextAssignment.roleName})` : ""}`
            )
            continue
        }
        ...
        if (previousAssignment.squadName !== nextAssignment.squadName) {
            ...
            movedLines.push(
                `🔁 ${userLabel}: ${previousAssignment.squadName} -> ${nextAssignment.squadName}`
            )
        }

        if (
            (previousAssignment.roleName ?? "") !==
            (nextAssignment.roleName ?? "")
        ) {
            roleChangedUserIds.push(userId)
            roleChanges[userId] = {
                previous: previousAssignment.roleName ?? "Unassigned",
                next: nextAssignment.roleName ?? "Unassigned",
            }
    ...
    for (const [userId, previousAssignment] of beforeAssignments) {
        ...
        removedLines.push(
            `🔴 ${userLabel} <- ${previousAssignment.squadName}${previousAssignment.roleName ? ` (${previousAssignment.roleName})` : ""}`
        )
    }
