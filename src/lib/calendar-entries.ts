...
export function buildCalendarDisplayEntries(input: {
    ...
    const eventEntries: CalendarDisplayEntry[] = input.events
        .filter((event) => {
            ...
            return (
                eventEnd >= input.rangeStart.getTime() &&
                eventStart <= input.rangeEnd.getTime()
            )
        })
        .map((event) => {
            const category = getEventCategoryPresentation(
                event,
    ...
    const manualEntries: CalendarDisplayEntry[] = expandCalendarItems(
        ...
        emoji: item.emoji,
        label: item.label,
        item: (input.calendarItems ?? []).find(
            (source) => source.id === item.sourceId
        )!,
    }))

    return [...eventEntries, ...manualEntries].sort(
        (left, right) =>
