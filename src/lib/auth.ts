...
const getAllGuildsInternalReference = makeFunctionReference<"query">(
    "guilds:listAllInternal"
)
...
export const getLoggedInUser = cache(
    async function getLoggedInUser(): Promise<AppUser | null> {
...
export const getVisibleGuildsForLoggedInUser = cache(
    async function getVisibleGuildsForLoggedInUser(): Promise<Guild[]> {
        const user = await getLoggedInUser()
        const session = await getSession()
        if (!user) {
            return []
        ...
        try {
            const sessionGuilds = (session?.discordGuilds ?? []).map(
                (guild) => ({
                    id: guild.id,
                    discordId: guild.id,
                    name: "",
...
export async function syncManagedGuildsForCurrentPlayer(
    userId: string,
    guilds: {
        id: string
        name: string
...
export async function setPrimaryGuildForCurrentPlayer(
    userId: string,
    guildId: string
) {
