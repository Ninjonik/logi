...
const SUPERADMIN_CONFIG_PATH = path.join(
    process.cwd(),
    "config",
    "superadmins.json"
)
...
const readSuperadminConfig = cache(
    async function readSuperadminConfig(): Promise<SuperadminConfig> {
        try {
...
export async function isSuperadminDiscordId(
