...
const listPublicClansReference = makeFunctionReference<"query">(
    "publicProfiles:listClans"
)
const listPublicMatchesReference = makeFunctionReference<"query">(
    "publicProfiles:listMatches"
)
...
export type PublicPlayerProfile = Awaited<
    ReturnType<typeof getPublicPlayerProfile>
>
    ...
    return await cachedRead(
        ...
        async () =>
            (await fetchQuery(getPublicPlayerReference, { playerId })) as {
                ...
                name: string
                avatar: string
                clans: Array<{ id: string; name: string; avatar: string }>
                stats: {
                    matches: number
                    kills: number
                    deaths: number
        ...
        async () =>
            (await fetchQuery(getPublicClanReference, { guildId })) as {
                ...
                recentMatches: Array<{
                    ...
                    gameEnd: string
                    mapName: string
                    score: { axis: number; allied: number }
                    outcome?: string
                    category?: string
                }>
                updatedAt: string
