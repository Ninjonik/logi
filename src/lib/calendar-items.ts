...
function expandCalendarItem(
    ...
    while (
        ...
        if (currentStart.getTime() >= baseStart.getTime()) {
            if (
                recurrence.until &&
                currentStart.getTime() > new Date(recurrence.until).getTime()
            ) {
                break
            }
        ...
        occurrenceIndex += 1
        currentStart = getNextOccurrenceStart(
            baseStart,
            recurrence,
            occurrenceIndex
        )
...
function overlapsRange(
    ...
    return (
        endAt.getTime() >= rangeStart.getTime() &&
...
function getNextOccurrenceStart(
    ...
    switch (recurrence.frequency) {
        ...
        case "monthly_date":
            return getMonthlyDateOccurrence(
                baseStart,
                recurrence.interval,
                occurrenceIndex
            )
        case "monthly_nth_weekday":
            return getMonthlyNthWeekdayOccurrence(
                baseStart,
                recurrence.interval,
                occurrenceIndex
            )
        case "yearly":
            return getYearlyOccurrence(
                baseStart,
                recurrence.interval,
                occurrenceIndex
            )
...
function getMonthlyDateOccurrence(
    baseStart: Date,
    interval: number,
    occurrenceIndex: number
) {
    const targetMonthIndex =
        baseStart.getUTCMonth() + interval * occurrenceIndex
...
function getMonthlyNthWeekdayOccurrence(
    baseStart: Date,
    interval: number,
    occurrenceIndex: number
) {
