...
export function parseCollectionQuery(
    ...
    if (
        ...
        return {
            error: "offset must be a non-negative integer and limit must be between 1 and 100.",
        }
    }
    ...
    for (const [name, value] of params) {
        ...
        if (!match)
            return {
                error: "Filter names must use filter[field] or filter[parent.field].",
            }
...
export function isCollectionQueryError(
    query: CollectionQuery | { error: string }
): query is { error: string } {
...
export function getNamedCollection(
