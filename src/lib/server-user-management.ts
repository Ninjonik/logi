...
export async function getServerUserAssignments(
    serverId: string
): Promise<ServerUserAssignment[]> {
...
export async function getEligibleUsersForServer(
    server: Guild,
    ...
    return currentUsers.map((user) => {
        const existingHere = assignments.find(
            (assignment) => assignment.userId === user.discordId
        )
        const canJoinAsMember =
            (!user.guildId || user.guildId === server.discordId) &&
            existingHere?.type !== "mercenary"
        const canJoinAsMercenary =
            existingHere?.type !== "member" &&
            existingHere?.type !== "reserve_member"
...
export async function savePlayerNote(input: { userId: string; note?: string }) {
