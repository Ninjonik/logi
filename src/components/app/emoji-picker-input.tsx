...
export function EmojiPickerInput({
    ...
    const customEmojiMap = useMemo(
        () =>
            new Map(
                customEmojis.map((emoji) => [emoji.id.toLowerCase(), emoji.id])
            ),
        [customEmojis]
    )
    const selectedCustomEmoji = useMemo(
        () =>
            customEmojis.find(
                (emoji) => emoji.id.toLowerCase() === value?.toLowerCase()
            ),
        [customEmojis, value]
    )
    ...
    const pickerProps = useMemo<PickerProps>(
        () => ({
            ...
            categories: [
                {
                    ...
                    name: labels.suggestedCategory,
                },
                { category: Categories.CUSTOM, name: labels.customCategory },
                {
                    category: Categories.SMILEYS_PEOPLE,
                    name: labels.smileysPeopleCategory,
                },
                {
                    category: Categories.ANIMALS_NATURE,
                    name: labels.animalsNatureCategory,
                },
                {
                    category: Categories.FOOD_DRINK,
                    name: labels.foodDrinkCategory,
                },
                {
                    category: Categories.TRAVEL_PLACES,
                    name: labels.travelPlacesCategory,
                },
                {
                    category: Categories.ACTIVITIES,
                    name: labels.activitiesCategory,
                },
            ...
            onEmojiClick: (emojiData: EmojiClickData) => {
                onChange(
                    emojiData.isCustom
                        ? (customEmojiMap.get(emojiData.unified) ??
                              emojiData.unified)
                        : emojiData.emoji
                )
                setOpen(false)
    ...
    return (
        <div className={cn("flex items-center gap-2", className)}>
            <Popover open={open} onOpenChange={setOpen}>
                    ...
                    >
                        <span className="flex min-w-0 items-center gap-2 truncate">
                            {selectedCustomEmoji?.imageUrl ? (
                                <img
                                    src={selectedCustomEmoji.imageUrl}
                                    alt=""
                ...
                <PopoverContent
                    align="start"
                    className="border-border/60 w-[352px] rounded-xl p-0"
                >
                    <div className="border-border/60 text-muted-foreground border-b px-3 py-2 text-sm">
                        {labels.pickEmoji}
                    </div>
