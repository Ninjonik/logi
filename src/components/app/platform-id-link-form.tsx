...
export function PlatformIdLinkForm({
    ...
    const selectedPlatform = platform
        ? dictionary.platformIdLink[platform]
        : null
    ...
    if (expired) {
        return (
            <div className="border-border/60 bg-muted/20 text-muted-foreground rounded-2xl border p-4 text-sm">
        ...
        return (
            <div className="flex min-h-[24rem] items-center justify-center">
                <div className="flex flex-col items-center text-center">
                    <CheckCircle2
                        className="size-28 text-emerald-500 sm:size-36"
                        strokeWidth={1.5}
                    />
                    <p className="text-muted-foreground mt-6 max-w-sm text-sm sm:text-base">
                        {successMessage}
                    </p>
                </div>
            </div>
        ...
        <div className="space-y-4">
            <div className="space-y-2">
                <h1 className="text-2xl font-semibold tracking-tight">
                    {dictionary.platformIdLink.title}
                </h1>
            ...
            <div className="border-border/60 bg-muted/20 text-muted-foreground rounded-2xl border p-4 text-sm">
                {dictionary.platformIdLink.userPrefix}{" "}
            ...
            <div className="space-y-2">
                ...
                >
                    <SelectTrigger className="rounded-xl">
                        <SelectValue
                            placeholder={
                                dictionary.platformIdLink.platformPlaceholder
                            }
                        />
                    </SelectTrigger>
                    <SelectContent>
                        <SelectItem value="steam">
                            {dictionary.platformIdLink.steam.label}
                        </SelectItem>
                        <SelectItem value="epic">
                            {dictionary.platformIdLink.epic.label}
                        </SelectItem>
                        <SelectItem value="xbox">
                            {dictionary.platformIdLink.xbox.label}
                        </SelectItem>
                        <SelectItem value="playstation">
                            {dictionary.platformIdLink.playstation.label}
                        </SelectItem>
                    </SelectContent>
                </Select>
            </div>
                ...
                <>
                    <div className="border-border/60 bg-muted/20 text-muted-foreground rounded-2xl border p-4 text-sm">
                        ...
                        <a
                            href={
                                selectedPlatform
                                    ? PLATFORM_GUIDES[platform as PlatformKey]
                                    : "#"
                            }
                            target="_blank"
                            rel="noreferrer"
                        ...
                        >
                            {selectedPlatform.guideLabel ??
                                dictionary.platformIdLink.guideLabel}
                        </a>
                    ...
                    <div className="space-y-2">
                        ...
                        {platformId.trim() ? (
                            <p className="text-muted-foreground text-sm">
                                {dictionary.shared.detectedPlatformId.replace(
                                    "{platform}",
                                    `${selectedPlatform.label} ID`
                                )}
                            </p>
                        ) : null}
                    </div>
                    ...
                    <Button
                        ...
                        disabled={isPending || !platformId.trim()}
                    >
