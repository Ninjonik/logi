        ...
        () =>
            Object.fromEntries(
                attendees.map((attendee) => [
                    attendee.userId,
                    attendee.completed ?? "pending",
                ])
            ) as Record<string, CompletionState>,
        [attendees]
    )
    const [completionByUserId, setCompletionByUserId] =
        useState<Record<string, CompletionState>>(initialStates)

    const allResolved = attendees.every(
        (attendee) =>
            completionByUserId[attendee.userId] === "passed" ||
            completionByUserId[attendee.userId] === "failed"
    )
    ...
    function handleSubmit() {
        startTransition(async () => {
            const participants = attendees.map((attendee) => ({
                ...
                completed: completionByUserId[attendee.userId] as
                    "passed" | "failed",
            }))
            ...
            toast.success(
                dictionary.event.trainingCompletionSuccess
    ...
    return (
        <Dialog open={isOpen} onOpenChange={handleOpenChange}>
            ...
            <DialogContent className="max-w-2xl rounded-2xl">
                <DialogHeader>
                    <DialogTitle>
                        {dictionary.event.trainingCompletionTitle}
                    </DialogTitle>
                    <DialogDescription>
                        {dictionary.event.trainingCompletionDescription}
                    </DialogDescription>
                </DialogHeader>
                <div className="max-h-[60vh] space-y-3 overflow-y-auto pr-1">
                    {attendees.length === 0 ? (
                        ...
                        attendees.map((attendee) => {
                            const selected =
                                completionByUserId[attendee.userId] ?? "pending"
                ...
                <DialogFooter>
                    ...
                    <Button
                        ...
                        disabled={
                            isPending || attendees.length === 0 || !allResolved
                        }
                    >
                        {isPending ? (
