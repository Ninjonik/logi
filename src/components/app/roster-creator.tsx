...
function getTrackedSignupUserIds(
    ...
    return new Set(
        users
            .filter(
                (user) =>
                    trackedUserIds.has(user.discordId) ||
                    trackedUserIds.has(user.id)
            )
...
export function RosterCreator({
    ...
    const draftRoster = useMemo(() => {
        ...
        const trackedUserIds = getTrackedSignupUserIds(
            selectedEvent,
            userAssignments,
            users
        )
        ...
        const attendingUserIds = new Set(
            users
                ...
                .filter(
                    (user) =>
                        attendingSignupIds.has(user.discordId) ||
                        attendingSignupIds.has(user.id)
                )
                .map((user) => user.discordId)
        )
        const reservePlayerIds = users
            ...
            .sort(
                (a, b) =>
                    getUserScoreForGuild(b, serverId) -
                        getUserScoreForGuild(a, serverId) ||
    ...
    return (
        <div className="space-y-6">
            {showSetupCard ? (
                <Card className="border-border/70 bg-card rounded-2xl">
                    <CardHeader>
                        <CardTitle className="text-lg">
                            {dictionary.roster.setupRoster}
                        </CardTitle>
                    </CardHeader>
            ...
            {draftRoster && selectedEvent ? (
                ...
                <div className="border-border/50 bg-muted/20 text-muted-foreground flex h-64 flex-col items-center justify-center rounded-3xl border-2 border-dashed">
