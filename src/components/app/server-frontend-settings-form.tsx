...
type EditableCalendarItem = {
    ...
    allDay: boolean
    recurrenceFrequency:
        "none" | "weekly" | "monthly_date" | "monthly_nth_weekday" | "yearly"
...
function toEditableCalendarItems(
    items: Guild["calendarItems"]
...
function combineDateAndTime(
    ...
    if (allDay) {
        return new Date(
            `${date}T${endOfDay ? "23:59" : "00:00"}:00`
        ).toISOString()
    }
    return new Date(
        `${date}T${time || (endOfDay ? "23:59" : "00:00")}:00`
    ).toISOString()
}

function toPersistedCalendarItems(
    items: EditableCalendarItem[]
): CalendarItem[] {
    return items.map((item) => ({
        id: item.id,
        guildId: "",
        ...
        recurrence:
            item.recurrenceFrequency === "none"
                ...
                : {
                      ...
                      interval: Math.max(
                          1,
                          Number.parseInt(item.recurrenceInterval || "1", 10)
                      ),
...
export function ServerFrontendSettingsForm({
    ...
    const [calendarItems, setCalendarItems] = useState<EditableCalendarItem[]>(
    ...
    function patchCategory(
        categoryId: string,
        patch: Partial<EditableEventCategory>
    ) {
    ...
    function patchCalendarItem(
        itemId: string,
        patch: Partial<EditableCalendarItem>
    ) {
