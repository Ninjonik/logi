    ...
    function handleSubmit() {
        startTransition(async () => {
            const response = await fetch(
                `/api/servers/${serverId}/users/auto-link-platform-ids`,
                {
                    method: "POST",
            ...
            if (
                (body.ambiguousUsers ?? 0) > 0 ||
                (body.conflictedUsers ?? 0) > 0 ||
                (body.failedEvents ?? 0) > 0
            ) {
                toast.error(
                    dictionary.userManagement.autoLinkPlatformIdsPartial
                        .replace(
                            "{ambiguous}",
                            String(body.ambiguousUsers ?? 0)
                        )
                        .replace(
                            "{conflicts}",
                            String(body.conflictedUsers ?? 0)
                        )
            ...
            <DialogContent className="rounded-2xl">
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-muted-foreground text-sm">
                        {dictionary.userManagement.autoLinkPlatformIdsHint}
                    </p>
                </div>
                <DialogFooter>
                    <Button
                    ...
                    <Button
                        ...
                        disabled={isPending || !normalizedClanTag}
                    >
                        {isPending ? (
