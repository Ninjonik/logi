...
function getDisplayMapName(mapId: string) {
    ...
    if (
        upstreamName &&
        /^[A-Za-z.\s]+$/.test(upstreamName) &&
        upstreamName.length < map.name.length
    ) {
        return upstreamName
    }
...
function resolvePointValue(
    ...
    return points.find((point) =>
        valueMode === "id"
            ? point.id === value
            : point.label.toLowerCase() === value.toLowerCase() ||
              point.grid.toLowerCase() === value.toLowerCase()
    )
}
...
export function HllMapSelector({
    ...
    return (
        <div className="flex w-full min-w-0 flex-row flex-wrap items-end gap-2 overflow-x-hidden">
            ...
            {includePoint ? (
                <div className="min-w-48 flex-1 space-y-1.5">
                    ...
                    <SearchableSelect
                        ...
                        noResults={labels.noResults}
                        options={(selectedMap?.strongpoints ?? []).map(
                            (point) => ({
                                value: point.id,
                                label: point.label,
