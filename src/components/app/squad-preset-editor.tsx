...
export function SquadPresetEditor({
    ...
    const [renderDraftSquads, setRenderDraftSquads] = useState<EditorSquad[]>(
        () => toEditorSquads(squads)
    )
    ...
    function moveSquad(index: number, direction: -1 | 1) {
        ...
        draftSquadsRef.current = next.map((squad, order) => ({
            ...squad,
            order,
        }))
        syncRenderedDraft()
    ...
    function updateRole(
        squadIndex: number,
        roleIndex: number,
        field: "name" | "count" | "note" | "icon" | "color",
        value: string | number
    ) {
    ...
    function removeRole(squadIndex: number, roleIndex: number) {
        const squad = draftSquadsRef.current[squadIndex]
        if (!squad) return
        squad.roles = squad.roles.filter(
            (_, currentRoleIndex) => currentRoleIndex !== roleIndex
        )
        syncRenderedDraft()
    }
    ...
    async function handleSave() {
        ...
        try {
            const response = await fetch(
                createMode
                    ? `/api/servers/${serverId}/squad-presets`
                    : `/api/servers/${serverId}/squad-presets/${presetId}`,
                {
                ...
                <div>
                    ...
                    {isEditing ? (
                        ...
                        <div className="border-border/60 bg-muted/30 rounded-xl border px-4 py-3 text-sm">
                            {renderDraftName ||
                                dictionary.presets.untitledPreset}
                        </div>
                    )}
                </div>
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between gap-2">
                                        ...
                                        <Button
                                            ...
                                            className="rounded-xl"
                                            onClick={() =>
                                                removeSquad(squadIndex)
                                            }
                                        >
                                            <Trash2 className="size-4" />
                                        </Button>
                                    </div>
                                    <div className="grid grid-cols-[auto_auto_56px_72px_56px_auto_auto] items-center gap-2">
                                        <Input
                                    ...
                                    <div className="space-y-3">
                                        {squad.roles.map((role, roleIndex) => (
                                            <div
                                                key={role.editorId}
                                                className="border-border/60 rounded-xl border p-2.5"
                                            >
                                                ...
                                                <div className="grid grid-cols-[2fr_60px_56px_1fr] items-center gap-2">
                                                    <Input
