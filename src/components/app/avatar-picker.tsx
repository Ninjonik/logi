"use client"

import { Camera, Loader2, Upload } from "lucide-react"
import { toast } from "sonner"
import * as React from "react"

import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import { uploadFileToConvex } from "@/lib/client-uploads"
import { Button } from "@/components/ui/button"
import { cn } from "@/lib/utils"

export function AvatarPicker({
    value,
    onChange,
    fallback,
    label,
    buttonLabel,
    uploadMessages,
    disabled,
    className,
}: {
    value: string
    onChange: (value: string) => void
    fallback: string
    label: string
    buttonLabel: string
    uploadMessages?: {
        prepareUploadError: string
        uploadImageError: string
        readImageUrlError: string
    }
    disabled?: boolean
    className?: string
}) {
    const inputRef = React.useRef<HTMLInputElement>(null)
    const [isUploading, setIsUploading] = React.useState(false)

    async function handleFiles(files: FileList | null) {
        const file = files?.[0]
        if (!file) return

        setIsUploading(true)
        try {
            const nextUpload = await uploadFileToConvex(file, {
                prepareUploadError: uploadMessages?.prepareUploadError,
                uploadFileError: uploadMessages?.uploadImageError,
                readFileUrlError: uploadMessages?.readImageUrlError,
            })
            const nextUrl = nextUpload.url
            onChange(nextUrl)
            toast.success(label)
        } catch (error) {
            toast.error(
                error instanceof Error
                    ? error.message
                    : (uploadMessages?.uploadImageError ??
                          "Unable to upload the image.")
            )
        } finally {
            setIsUploading(false)
            if (inputRef.current) {
                inputRef.current.value = ""
            }
        }
    }

    return (
        <div className={cn("flex items-center gap-4", className)}>
            <div className="relative">
                <Avatar className="border-border/60 size-20 rounded-2xl border">
                    <AvatarImage src={value} alt={label} />
                    <AvatarFallback className="rounded-2xl text-lg font-semibold">
                        {fallback}
                    </AvatarFallback>
                </Avatar>
                <div className="bg-background absolute -right-2 -bottom-2 rounded-full border p-2 shadow-sm">
                    {isUploading ? (
                        <Loader2 className="size-4 animate-spin" />
                    ) : (
                        <Camera className="size-4" />
                    )}
                </div>
            </div>
            <div className="space-y-2">
                <div className="text-sm font-medium">{label}</div>
                <Button
                    type="button"
                    variant="outline"
                    className="rounded-xl"
                    disabled={disabled || isUploading}
                    onClick={() => inputRef.current?.click()}
                >
                    <Upload className="size-4" />
                    {isUploading ? `${buttonLabel}...` : buttonLabel}
                </Button>
                <input
                    ref={inputRef}
                    type="file"
                    accept="image/*"
                    className="hidden"
                    onChange={(event) => {
                        void handleFiles(event.target.files)
                    }}
                />
            </div>
        </div>
    )
}
