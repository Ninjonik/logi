    ...
    <Card className="min-w-0 overflow-hidden rounded-xl border-border/60">
      ...
      <CardContent className="space-y-3 overflow-x-hidden px-3 pb-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="min-w-0 space-y-1.5"><Label className="text-xs">{dictionary.stratmaps.stroke}</Label><Input type="color" value={strokeColor} onChange={(event) => onStrokeColorChange(event.target.value)} className="h-9 min-w-0 overflow-hidden rounded-lg p-1" /></div>
          <div className="min-w-0 space-y-1.5"><Label className="text-xs">{dictionary.stratmaps.fill}</Label><Input type="color" value={fillColor.slice(0, 7)} onChange={(event) => onFillColorChange(`${event.target.value}33`)} className="h-9 min-w-0 overflow-hidden rounded-lg p-1" /></div>
        </div>
