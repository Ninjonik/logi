...
export function RenderedElement({
    ...
    const scale =
        element.kind === "icon"
            ? getElementDragScale(selected, hovered, dragging)
            : 1
    ...
    return (
        <g
            ...
            style={{
                cursor: mode === "view" ? "pointer" : "move",
                userSelect: "none",
                WebkitUserSelect: "none",
            }}
            transform={transform}
        >
            {element.kind === "icon" && icon ? (
                <IconElement
                    element={element}
            ...
            {element.kind === "line" ? (
                <LineElement element={element} dashArray={dashArray} />
            ) : null}
            {element.kind === "rectangle" ? (
                <RectangleElement element={element} dashArray={dashArray} />
            ) : null}
            {element.kind === "ellipse" ? (
                <EllipseElement element={element} dashArray={dashArray} />
            ) : null}
            ...
            {element.kind === "icon" && showSpawnRanges ? (
                <IconRangeElement element={element} />
            ) : null}
        </g>
    )
}
...
function getElementTransform(element: StratmapElement, scale: number) {
    ...
    if (element.kind === "text") {
        return rotation
            ? `rotate(${rotation} ${element.x} ${element.y})`
            : undefined
    }
...
function TextElement({
    ...
    element: Extract<StratmapElement, { kind: "text" }>
}) {
...
function LineElement({
    ...
    const startArrow = buildArrowDecoration(
        element.startStyle,
        element.points,
        "start",
        strokeWidth
    )
    const endArrow = buildArrowDecoration(
        element.endStyle,
        element.points,
        "end",
        strokeWidth
    ...
    const startMarker =
        startArrow && typeof startArrow === "object" ? startArrow : null
    ...
    return (
        <>
            <PathElement
                ...
                dashArray={dashArray}
            />
            {typeof startArrow === "string" ? (
                <polygon points={startArrow} fill={stroke} />
            ) : null}
            {typeof endArrow === "string" ? (
                <polygon points={endArrow} fill={stroke} />
            ) : null}
            {startMarker && element.startStyle === "circle" ? (
                <circle
                    cx={startMarker.anchor.x}
                    cy={startMarker.anchor.y}
                    r={startMarker.size * 0.34}
                    fill={stroke}
                />
            ) : null}
            {endMarker && element.endStyle === "circle" ? (
                <circle
                    cx={endMarker.anchor.x}
                    cy={endMarker.anchor.y}
                    r={endMarker.size * 0.34}
                    fill={stroke}
                />
            ) : null}
            {startMarker && element.startStyle === "square" ? (
                <rect
                    x={startMarker.anchor.x - startMarker.size * 0.3}
                    y={startMarker.anchor.y - startMarker.size * 0.3}
                    width={startMarker.size * 0.6}
                    height={startMarker.size * 0.6}
                    fill={stroke}
                />
            ) : null}
            {endMarker && element.endStyle === "square" ? (
                <rect
                    x={endMarker.anchor.x - endMarker.size * 0.3}
                    y={endMarker.anchor.y - endMarker.size * 0.3}
                    width={endMarker.size * 0.6}
                    height={endMarker.size * 0.6}
                    fill={stroke}
                />
            ) : null}
