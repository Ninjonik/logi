    ...
    return (
        <>
            {pings
                .filter(
                    (ping) =>
                        now - new Date(ping.createdAt).getTime() <
                        PING_DURATION_MS
                )
                .map((ping) => (
    ...
    return (
        <g>
            <circle
                ...
                r={6 + progress * 12}
                fill={ping.color}
                opacity={Math.max(0, 0.75 - progress * 0.75)}
            />
            {[
                {
                    begin: 0,
                ...
                const ringProgress = Math.max(
                    0,
                    Math.min(1, (progress - ringStart) / (1 - ringStart))
                )
                if (ringProgress <= 0) {
                    return null
                }
                ...
                return (
                    <circle
                        ...
                        strokeWidth={
                            ring.widthFrom +
                            (ring.widthTo - ring.widthFrom) * ringProgress
                        }
                        opacity={Math.max(
                            0,
                            ring.opacityFrom - ringProgress * ring.opacityFrom
                        )}
                    />
                )
            })}
