...
export function getActiveSlide(state: StratmapState, selectedSlideId: string) {
    return (
        state.slides.find((slide) => slide.id === selectedSlideId) ??
        state.slides[0]
    )
}
...
export function clampViewport(
    ...
    const minWidth = Math.min(
        canvas.width,
        Math.max(MIN_VIEWPORT_SIZE * aspectRatio, MIN_VIEWPORT_SIZE)
    )
...
export function getSvgViewportMetrics(
...
export function getPointerPoint(
    ...
    const { bounds, width, height, offsetX, offsetY } = getSvgViewportMetrics(
        svgElement,
        viewport
    )
...
export function deriveStrongpointRadius(
    rects: Array<{ width: number; height: number }>
) {
    return Math.max(
        ...rects.map((rect) => Math.min(rect.width, rect.height) * 0.28),
        24
    )
}
...
export function updateSlide(
    state: StratmapState,
    slideId: string,
    updater: (slide: StratmapSlide) => StratmapSlide
) {
    return {
        ...state,
...
export function getPathDistance(points: Point[]) {
    return points
        ...
        .reduce(
            (sum, point, index) =>
                sum + getPointDistance(points[index]!, point),
            0
        )
}
...
export function getArrowDecorationVector(
    points: Point[],
    edge: "start" | "end"
) {
...
export function buildArrowDecoration(
    style: StratmapArrowStyle | undefined,
    points: Point[],
    edge: "start" | "end",
    strokeWidth: number
) {
...
export function isAreaDrag(
    active: { start: Point; current: Point },
    threshold = 10
) {
    return (
        Math.abs(active.current.x - active.start.x) > threshold ||
        Math.abs(active.current.y - active.start.y) > threshold
    )
}
...
export function filterLivePings(slide: StratmapSlide, maxAgeMs: number) {
    const now = Date.now()
    return slide.pings.filter(
        (ping) => now - new Date(ping.createdAt).getTime() < maxAgeMs
    )
}

export function getOverlayStrongpoints(
    state: StratmapOverlaySettings,
    pointIds: string[]
) {
    return state.showAllStrongpoints
        ? new Set(pointIds)
        : new Set(state.visibleStrongpointIds)
}

export function getOverlayItems<T>(
    items: { a: T[]; b: T[] } | undefined,
...
export function isIconElement(
    element: StratmapElement | null
): element is StratmapIconElement {
    return element?.kind === "icon"
}

export function getElementDragScale(
    selected: boolean,
    hovered: boolean,
    dragging: boolean
) {
...
export function getStratmapMetaSignature(
    stratmap: Pick<
        StratmapRecord,
        "title" | "description" | "baseMapId" | "side" | "strongpointId"
    >
) {
