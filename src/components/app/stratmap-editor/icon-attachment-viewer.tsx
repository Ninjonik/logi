...
export function IconAttachmentViewer({
    request,
}: {
    request: IconAttachmentViewerRequest | null
}) {
    if (!request || !request.attachments.length) return null
    return (
        <RequestedIconAttachmentViewer
...
function RequestedIconAttachmentViewer({
    request,
}: {
    request: IconAttachmentViewerRequest
}) {
    ...
    const [index, setIndex] = useState(() =>
        Math.max(
            ...
            attachments.findIndex(
                (attachment) => attachment.url === request.mainAttachmentUrl
            )
        )
    )
    ...
    return (
        <PhotoSlider
            ...
            overlayRender={({ index: activeIndex }) => (
                <LightboxCaption
                    text={
                        attachments[activeIndex]?.description?.trim() ||
                        request.fallbackNote?.trim() ||
                        ""
                    }
                />
            )}
