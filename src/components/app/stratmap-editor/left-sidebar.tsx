...
function SlidesPanel({
    ...
    const actions = canAdmin ? (
        <div className="flex gap-0.5">
            <EditorIconButton
                icon={Plus}
                label={dictionary.stratmaps.createSlideAction ?? "Add slide"}
                onClick={onAddSlide}
            />
            <EditorIconButton
                icon={Copy}
    ...
    return (
        <EditorPanel
            title={dictionary.stratmaps.slides}
            icon={Layers3}
            action={actions}
        >
                    ...
                    >
                        {canAdmin ? (
                            <EditorInput
                                value={slide.name}
                                onChange={(event) =>
                                    onRenameSlide(slide.id, event.target.value)
                                }
                                onFocus={() => onSelectSlide(slide.id)}
                                className="h-5 min-w-0 flex-1 border-0 bg-transparent px-1 shadow-none focus:ring-0"
                            />
                        ) : (
                            <button
                                type="button"
                                className="min-w-0 flex-1 truncate px-1 text-left text-[10px]"
                        ...
                        {canAdmin ? (
                            <div className="ml-0.5 flex gap-0.5 opacity-50 group-focus-within:opacity-100 group-hover:opacity-100">
                                <EditorIconButton
                                    icon={ChevronUp}
                                    label="Move slide up"
...
function StrongpointsPanel({
    ...
    return (
        <EditorPanel title={dictionary.stratmaps.visibleStrongpoints}>
            <div className="max-h-48 [scrollbar-width:thin] space-y-px overflow-y-auto pr-0.5">
                {selectedMap?.strongpoints.map((point) => {
                    const visible =
                        activeOverlays.showAllStrongpoints ||
                        activeOverlays.visibleStrongpointIds.includes(point.id)
                    return (
                        <button
                            key={point.id}
                            type="button"
                            ...
                            className={cn(
                                "hover:bg-muted/50 flex h-6 w-full items-center gap-1.5 rounded-[3px] px-1.5 text-left text-[10px]",
