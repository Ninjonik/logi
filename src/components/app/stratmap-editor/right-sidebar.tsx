...
export function StratmapRightSidebar(props: RightSidebarProps) {
    ...
    const selectedIcon =
        selectedElement?.kind === "icon" ? selectedElement : null
    const attachments =
        selectedIcon?.attachments?.filter((attachment) => attachment.url) ?? []

    return (
        <aside className="min-w-0 [scrollbar-width:thin] space-y-1 overflow-x-hidden overflow-y-auto pl-0.5">
            {canAdmin ? (
            ...
            {mode === "edit" && tool !== "select" ? (
                <EditorPanel
                    title={dictionary.stratmaps.toolProperties}
            ...
            {canEdit && selectedElement ? (
                <EditorPanel
                    title={
                        selectedElement.kind === "icon"
                            ? dictionary.stratmaps.selectedIcon
                            : dictionary.stratmaps.selectedElement
                    }
                ...
                >
                    {selectionOpen ? (
                        <SelectionInspector
                            ...
                            onElementChange={onSelectedElementChange}
                        />
                    ) : null}
                </EditorPanel>
            ) : null}
            ...
            {selectedIcon ? (
                <EditorPanel
                    title={dictionary.stratmaps.images}
                    icon={ImageIcon}
                    action={
                        <CollapseButton
                            open={imagesOpen}
                ...
                >
                    {imagesOpen ? (
                        <AttachmentGallery
                            ...
                            onUpload={onUpload}
                            onMainAttachmentChange={(url) =>
                                onSelectedElementChange((element) =>
                                    element.kind === "icon"
                                        ? { ...element, mainAttachmentUrl: url }
                                        : element
                                )
                            }
                            onDescriptionChange={
                                    ...
                                    ? (index) =>
                                          onSelectedElementChange((element) => {
                                              if (element.kind !== "icon")
                                                  return element
                                              const removedUrl =
                                                  element.attachments?.[index]
                                                      ?.url
                                              const attachmentsAfterRemoval = (
                                                  element.attachments ?? []
                                              ).filter(
                                                  (_, attachmentIndex) =>
                                              ...
                                              return {
                                                  ...element,
                                                  attachments:
                                                      attachmentsAfterRemoval,
                                                  mainAttachmentUrl:
                                                      element.mainAttachmentUrl ===
                                                      removedUrl
                                                          ? attachmentsAfterRemoval[0]
                                                                ?.url
                                                          : element.mainAttachmentUrl,
                                              }
                                          })
...
function CollapseButton({
    ...
    return (
        <EditorIconButton
            ...
            className={`size-5 border-0 bg-transparent transition-transform ${open ? "rotate-180" : ""}`}
            onClick={onClick}
        />
    )
}
