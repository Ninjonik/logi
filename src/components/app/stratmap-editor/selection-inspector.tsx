        ...
        <div className="flex flex-wrap gap-1">
            {MAPS_LET_LOOSE_COLORS.map((color) => (
                <button
                    ...
                    type="button"
                    className={cn(
                        "border-border/70 size-5 rounded-[2px] border",
                        value === color &&
...
function SectionCard({
    ...
    return (
        <div
            className="min-w-0 space-y-1.5 overflow-x-hidden"
        ...
        <SectionCard title={dictionary.stratmaps.selectedIcon}>
                    ...
                    onChange={(event) =>
                        onElementChange((element) =>
                            element.kind === "icon"
                                ? {
                                      ...element,
                                      size: Number(event.target.value) || 50,
                                  }
                                : element
                        )
                    }
            ...
            <EditorField label={dictionary.stratmaps.notes}>
                <EditorTextarea
                    ...
                    disabled={!canAdmin}
                    placeholder={dictionary.stratmaps.notePlaceholder}
                />
            </EditorField>
        </SectionCard>
    )
...
function ShapeInspector({
    ...
    selectedElement: Extract<
        StratmapElement,
        { kind: "rectangle" | "ellipse" | "polygon" | "freehand" }
    >
    onElementChange: (
        updater: (element: StratmapElement) => StratmapElement
    ) => void
                        ...
                        onElementChange((element) =>
                            element.id === selectedElement.id
                                ? {
                                      ...element,
                                      strokeColor: value,
                                      fillColor: element.fillColor?.startsWith(
                                          "rgba"
                                      )
                                          ? `${value}33`
                                          : element.fillColor,
            ...
            <div className="grid grid-cols-2 gap-1">
                <EditorField label={dictionary.stratmaps.strokeWidth}>
                    <EditorInput
                        ...
                        min={1}
                        max={24}
                        value={selectedElement.strokeWidth ?? 6}
                        onChange={(event) =>
                            onElementChange((element) =>
                                element.id === selectedElement.id
                                    ? {
                        ...
                        onChange={(event) =>
                            onElementChange((element) =>
                                element.id === selectedElement.id
                                    ? {
                                          ...element,
                                          fillColor: `${event.target.value}33`,
                                      }
                                    : element
                            )
                        }
                ...
                <EditorInput
                    ...
                    value={
                        ...
                        selectedElement.backgroundColor !== "transparent"
                            ? selectedElement.backgroundColor.slice(0, 7)
                            : "#000000"
                    }
                    onChange={(event) =>
                        onElementChange((element) =>
                            element.kind === "text"
                                ? {
                                      ...element,
                                      backgroundColor: `${event.target.value}cc`,
                                  }
                                : element
                        )
                    }
        ...
        <SectionCard title={dictionary.stratmaps.selectedElement}>
            ...
            <div className="grid grid-cols-2 gap-1">
                <EditorField label={dictionary.stratmaps.lineStart}>
                    <ArrowStyleSelect
                        value={selectedElement.startStyle ?? "none"}
                        onChange={(value) =>
                            onElementChange((element) =>
                                element.kind === "line"
                                    ? { ...element, startStyle: value }
                                    : element
                            )
                        }
                        dictionary={dictionary}
                ...
                <EditorField label={dictionary.stratmaps.lineEnd}>
                    <ArrowStyleSelect
                        value={selectedElement.endStyle ?? "arrow"}
                        onChange={(value) =>
                            onElementChange((element) =>
                                element.kind === "line"
                                    ? { ...element, endStyle: value }
                                    : element
                            )
                        }
                        dictionary={dictionary}
            ...
            <EditorToggle
                ...
                checked={selectedElement.showDistance ?? false}
                onCheckedChange={(checked) =>
                    onElementChange((element) =>
                        element.kind === "line"
                            ? { ...element, showDistance: checked }
                            : element
                    )
                }
                disabled={!canAdmin}
...
export function AttachmentGallery({
    ...
    return (
        <div className="space-y-1.5">
            {canAdmin && mode === "edit" ? (
                <label className="border-border/70 bg-background/70 hover:bg-accent hover:text-accent-foreground inline-flex h-6 cursor-pointer items-center justify-center gap-1 rounded-[3px] border px-2 text-[10px] font-medium transition-colors">
                    <ImagePlus className="size-3" />
                    {isUploading
                        ? dictionary.stratmaps.uploading
                        : dictionary.stratmaps.attachImages}
                    <input
                        type="file"
                        multiple
                        accept="image/*"
                    ...
                    <div className="flex flex-col gap-1.5">
                        {attachments.map((attachment, index) => (
                            <AttachmentCard
                                ...
                                isMain={
                                    (mainAttachmentUrl ??
                                        attachments[0]?.url) === attachment.url
                                }
                                fallbackNote={fallbackNote}
                                onOpen={() => openAttachment(attachment)}
                                onMainAttachmentChange={() =>
                                    onMainAttachmentChange(attachment.url)
                                }
...
function AttachmentCard({
    ...
    return (
        <div
            className={cn(
                "min-w-0 overflow-hidden rounded-[3px] border p-1.5",
                isMain ? "border-primary/70 bg-primary/5" : "border-border/60"
            )}
        >
            <div className="flex items-start justify-between gap-1">
                <div className="min-w-0 flex-1">
                    {previewable ? (
                        <div className="border-border/60 relative overflow-hidden rounded-[2px] border">
                            <button
                                type="button"
                                onClick={onOpen}
                        ...
                        <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="border-border/60 text-primary block rounded-md border px-2 py-1.5 text-[11px] underline-offset-2 hover:underline"
                        >
                    ...
                    <div className="mt-1 flex min-w-0 items-center justify-between gap-2">
                        <div className="text-muted-foreground truncate text-[9px]">
                            {attachment.filename ||
                                `${dictionary.stratmaps.images} ${index + 1}`}
                        </div>
                        {canAdmin && mode === "edit" ? (
                            <label className="text-foreground/80 flex shrink-0 cursor-pointer items-center gap-1 text-[9px] font-medium">
                                <input
                                    type="radio"
                                    name="stratmap-main-image"
                                    checked={isMain}
                                    onChange={onMainAttachmentChange}
            ...
            {mode === "edit" ? (
                <EditorField
                    label={dictionary.stratmaps.imageDescription}
                    className="mt-1"
                >
                    <DiscordMarkdownTextarea
                        ...
                        disabled={!canAdmin}
                        placeholder={
                            dictionary.stratmaps.imageDescriptionPlaceholder
                        }
                        height={116}
                        preview="edit"
                        compactToolbar
...
function getAttachmentCaption(
    attachment: StratmapElementAttachment | undefined,
    fallbackNote?: string
) {
...
function isPreviewableImage(attachment: StratmapElementAttachment) {
    ...
    return /\.(avif|bmp|gif|heic|jpeg|jpg|png|svg|webp)$/i.test(
