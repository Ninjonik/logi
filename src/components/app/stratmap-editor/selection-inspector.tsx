...
export function SelectionInspector({
  ...
  if (selectedElement.kind === "icon") return <IconInspector {...{ dictionary, canAdmin, strokeColor, selectedElement, isUploadingIconAttachments, onElementChange, onUpload }} />;
...
function IconInspector(props: { dictionary: Dictionary; canAdmin: boolean; strokeColor: string; selectedElement: Extract<StratmapElement, { kind: "icon" }>; isUploadingIconAttachments: boolean; onElementChange: (updater: (element: StratmapElement) => StratmapElement) => void; onUpload: (event: React.ChangeEvent<HTMLInputElement>) => void; }) {
  const { dictionary, canAdmin, strokeColor, selectedElement, isUploadingIconAttachments, onElementChange, onUpload } = props;
  return (
    <Card className="min-w-0 overflow-hidden rounded-xl border-border/60">
      ...
      <CardContent className="space-y-3 overflow-x-hidden px-3 pb-3">
        <div className="space-y-1.5"><Label className="text-xs">{dictionary.stratmaps.size}</Label><Input type="number" min={24} max={160} value={selectedElement.size} onChange={(event) => onElementChange((element) => element.kind === "icon" ? { ...element, size: Number(event.target.value) || 24 } : element)} disabled={!canAdmin} className="min-w-0 overflow-hidden h-9 rounded-lg text-sm" /></div>
        <div className="space-y-1.5"><Label className="text-xs">{dictionary.stratmaps.notes}</Label><Textarea value={selectedElement.note ?? ""} onChange={(event) => onElementChange((element) => element.kind === "icon" ? { ...element, note: event.target.value } : element)} disabled={!canAdmin} className="min-h-24 rounded-lg px-3 py-2 text-sm" placeholder={dictionary.stratmaps.notePlaceholder} /></div>
        {canAdmin ? <Button type="button" variant="outline" className="h-8 rounded-lg px-3 text-xs" onClick={() => onElementChange((element) => element.kind === "icon" ? { ...element, color: strokeColor } : element)}>{dictionary.stratmaps.stroke}</Button> : null}
        {!selectedElement.attachments?.length && canAdmin ? <AttachmentGallery dictionary={dictionary} attachments={[]} canAdmin={canAdmin} isUploading={isUploadingIconAttachments} onUpload={onUpload} onDescriptionChange={() => undefined} onRemove={() => undefined} /> : null}
      </CardContent>
    </Card>
  );
}
...
function AttachmentCard({
  ...
  return (
    <div className="min-w-0 overflow-hidden rounded-xl border border-border/60 p-3">
      <div className="flex items-start justify-between gap-3">
        ...
        {canAdmin ? <Button type="button" variant="ghost" size="icon" className="size-8 shrink-0 rounded-lg" onClick={onRemove}><Trash2 className="size-4" /></Button> : null}
      </div>
...
function TextInspector({ dictionary, canAdmin, selectedElement, onElementChange }: { dictionary: Dictionary; canAdmin: boolean; selectedElement: Extract<StratmapElement, { kind: "text" }>; onElementChange: (updater: (element: StratmapElement) => StratmapElement) => void; }) {
  return (
    <Card className="min-w-0 overflow-hidden rounded-xl border-border/60">
      <CardHeader className="px-3 py-3"><CardTitle className="text-base">{dictionary.stratmaps.selectedElement}</CardTitle></CardHeader>
      <CardContent className="space-y-3 overflow-x-hidden px-3 pb-3">
        <div className="space-y-1.5"><Label className="text-xs">{dictionary.stratmaps.text}</Label><Textarea value={selectedElement.text} onChange={(event) => onElementChange((element) => element.kind === "text" ? { ...element, text: event.target.value } : element)} disabled={!canAdmin} className="min-h-20 rounded-lg px-3 py-2 text-sm" /></div>
        <div className="space-y-1.5"><Label className="text-xs">{dictionary.stratmaps.fontSize}</Label><Input type="number" min={16} max={96} value={selectedElement.fontSize} onChange={(event) => onElementChange((element) => element.kind === "text" ? { ...element, fontSize: Number(event.target.value) || 16 } : element)} disabled={!canAdmin} className="min-w-0 overflow-hidden h-9 rounded-lg text-sm" /></div>
      </CardContent>
    </Card>
  );
}

function LineInspector({ dictionary, canAdmin, selectedElement, onElementChange }: { dictionary: Dictionary; canAdmin: boolean; selectedElement: Extract<StratmapElement, { kind: "line" }>; onElementChange: (updater: (element: StratmapElement) => StratmapElement) => void; }) {
  return (
    <Card className="min-w-0 overflow-hidden rounded-xl border-border/60">
      <CardHeader className="px-3 py-3"><CardTitle className="text-base">{dictionary.stratmaps.selectedElement}</CardTitle></CardHeader>
      <CardContent className="space-y-3 overflow-x-hidden px-3 pb-3">
        <div className="grid grid-cols-2 gap-2">
          <div className="min-w-0 space-y-1.5"><Label className="text-xs">{dictionary.stratmaps.lineStart}</Label><ArrowStyleSelect value={selectedElement.startStyle ?? "none"} onChange={(value) => onElementChange((element) => element.kind === "line" ? { ...element, startStyle: value } : element)} dictionary={dictionary} disabled={!canAdmin} /></div>
          <div className="min-w-0 space-y-1.5"><Label className="text-xs">{dictionary.stratmaps.lineEnd}</Label><ArrowStyleSelect value={selectedElement.endStyle ?? "none"} onChange={(value) => onElementChange((element) => element.kind === "line" ? { ...element, endStyle: value } : element)} dictionary={dictionary} disabled={!canAdmin} /></div>
        </div>
