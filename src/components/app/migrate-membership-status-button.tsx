    ...
    function handleSubmit() {
        startTransition(async () => {
            const response = await fetch(
                `/api/servers/${serverId}/users/migrate-membership-status`,
                {
                    method: "POST",
            ...
            if ((body.skippedUnassigned ?? 0) > 0) {
                toast.message(
                    dictionary.userManagement.membershipMigrationSkippedUnassigned.replace(
                        "{count}",
                        String(body.skippedUnassigned)
                    )
                )
            }
            ...
            if ((body.skippedUnchanged ?? 0) > 0) {
                toast.message(
                    dictionary.userManagement.membershipMigrationSkippedUnchanged.replace(
                        "{count}",
                        String(body.skippedUnchanged)
                    )
                )
            }
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <Select
                            value={target}
                            onValueChange={(value) =>
                                setTarget(value as MigrationTarget)
                            }
                        >
                    ...
                    <p className="text-muted-foreground text-sm">
                        {dictionary.userManagement.membershipMigrationHint}
                    </p>
                </div>
                <DialogFooter>
                    <Button
