    ...
    function handleSubmit() {
        startTransition(async () => {
            const response = await fetch(
                `/api/servers/${serverId}/users/merge`,
                {
                    method: "POST",
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="rounded-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {dictionary.userManagement.mergeUsersDescription}
                    </DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                    <div className="space-y-2">
                        <Label>
                            {dictionary.userManagement.mergeUsersPrimary}
                        </Label>
                        <EntitySelect
                            value={primaryUserId}
                            onChange={(value) => {
                            ...
                            options={users}
                            placeholder={
                                dictionary.userManagement.mergeUsersPickPrimary
                            }
                            allowNone={false}
                        />
                    </div>
                    <div className="space-y-2">
                        <Label>
                            {dictionary.userManagement.mergeUsersSecondary}
                        </Label>
                        <EntitySelect
                            value={secondaryUserId}
                            onChange={(value) =>
                                setSecondaryUserId(value ?? "")
                            }
                            options={secondaryOptions}
                            placeholder={
                                dictionary.userManagement
                                    .mergeUsersPickSecondary
                            }
                            allowNone={false}
                        />
                    </div>
                    <p className="text-muted-foreground text-sm">
                        {dictionary.userManagement.mergeUsersHint}
                    </p>
                </div>
                <DialogFooter>
                    <Button
