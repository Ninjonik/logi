    ...
    function handleSubmit() {
        startTransition(async () => {
            const response = await fetch(
                `/api/servers/${serverId}/users/link-missing-discord-ids`,
                {
                    method: "POST",
            ...
            if ((body.mergedUsers ?? 0) > 0) {
                toast.success(
                    dictionary.userManagement.linkMissingDiscordIdsMerged.replace(
                        "{count}",
                        String(body.mergedUsers ?? 0)
                    )
                )
            }
            ...
            <DialogContent className="rounded-2xl">
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-muted-foreground text-sm">
                        {dictionary.userManagement.linkMissingDiscordIdsHint}
                    </p>
                </div>
                <DialogFooter>
                    <Button
