...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!data) return NextResponse.json({ error: { code: "not_found", message: "Competition not found." } }, { status: 404 });
  ...
  const items = collection === "divisions" ? getNamedCollection(data, ["divisions"]) : null;
