...
const collections = new Set(["events", "groups", "rosters", "assignments", "calendarItems", "stratmaps", "topicPresets", "squadPresets", "matches", "articles"]);
...
export async function GET(request: Request, { params }: { params: Promise<{ collection: string }> }) {
  ...
  if (!collections.has(collection)) return NextResponse.json({ error: { code: "not_found", message: "Collection not found." } }, { status: 404 });
  ...
  if (!Array.isArray(items)) return NextResponse.json({ error: { code: "not_found", message: "Collection not found." } }, { status: 404 });
  return NextResponse.json({ data: paginateCollection(filterCollection(items as Array<Record<string, unknown>>, query.filters), query.offset, query.limit) });
}
