...
export async function POST(
  ...
  try {
    ...
    if (event.status === "concluded") return NextResponse.json({ error: "Concluded events cannot be resynced." }, { status: 400 });

    await requestForumTopicResync({ eventId });
    revalidateCacheEntries([appCacheTags.event(eventId), appCacheTags.events(serverId), appCacheTags.serverContext(serverId)]);
    return NextResponse.json({ ok: true });
  } catch (error) {
    logRouteError("events.resyncTopicThread", error);
    return NextResponse.json({ error: getUserSafeErrorMessage(error, "Unable to resync the topic thread.") }, { status: 400 });
  }
}
