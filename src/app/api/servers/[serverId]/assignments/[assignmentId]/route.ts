...
function getAssignmentErrorCode(error: unknown) {
    ...
    if (error.message.includes("already assigned to this server"))
        return "ALREADY_ASSIGNED"
    if (error.message.includes("already linked to another player"))
        return "PLATFORM_ALREADY_LINKED"
    return "UNKNOWN"
}

async function syncRolesSafely(
    input: Parameters<typeof syncDiscordRolesForAssignment>[0]
) {
    try {
...
export async function PATCH(
    ...
    try {
        ...
        const normalizedBody = {
            ...body,
            secondaryGroupIds: body.secondaryGroupIds.filter(
                (groupId) => groupId !== body.primaryGroupId
            ),
        }
        ...
        await syncRolesSafely({
            ...
            beforeSecondaryGroupIds:
                existingAssignment?.secondaryGroupIds ?? [],
        ...
        return NextResponse.json(
            {
                ...
                error: getUserSafeErrorMessage(
                    error,
                    "Unable to save the player assignment."
                ),
            },
            { status: 400 }
        )
    ...
    try {
        ...
        revalidateCacheEntries([
            ...
            existingAssignment?.userId
                ? appCacheTags.player(existingAssignment.userId)
                : undefined,
            appCacheTags.users(),
            appCacheTags.rosterImage(),
        ])
        ...
        return NextResponse.json(
            {
                ...
                error: getUserSafeErrorMessage(
                    error,
                    "Unable to delete the player assignment."
                ),
            },
            { status: 400 }
        )
