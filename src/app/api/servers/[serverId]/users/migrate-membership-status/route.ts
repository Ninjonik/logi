    ...
    try {
        ...
        if (!roleId || !target) {
            return NextResponse.json(
                { error: "Role and target status are required." },
                { status: 400 }
            )
        }

        const guildMembers = await fetchDiscordGuildMembers(
            context.server.discordId
        )
        const matchingDiscordUserIds = new Set(
            guildMembers
                .filter(
                    (member) => member.user?.id && member.roles.includes(roleId)
                )
        ...
        for (const userId of matchingDiscordUserIds) {
            ...
            if (
                assignment.type === nextAssignment.type &&
                assignment.status === nextAssignment.status
            ) {
                skippedUnchanged += 1
                continue
            }
            ...
            try {
                ...
                logRouteError(
                    "users.migrateMembershipStatus.discordRoleSync",
                    error
                )
            }
        ...
        return NextResponse.json(
            {
                error: getUserSafeErrorMessage(
                    error,
                    "Unable to migrate membership statuses."
                ),
            },
            { status: 400 }
        )
