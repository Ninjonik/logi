...
export async function POST(
    ...
    try {
        ...
        revalidateCacheEntries([
            ...
            ...result.touchedEventIds.map((eventId) =>
                appCacheTags.event(eventId)
            ),
            ...result.touchedEventIds.map((eventId) =>
                appCacheTags.match(eventId)
            ),
            ...result.touchedRosterIds.map((rosterId) =>
        ...
        logNextError("merge-users", "Failed to merge users", {
            serverId,
            error,
        })
        return NextResponse.json(
            { error: "Unable to merge users." },
            { status: 500 }
        )
    }
}
