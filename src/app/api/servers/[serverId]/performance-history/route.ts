...
export async function POST(_request: Request, { params }: { params: Promise<{ serverId: string }> }) {
  ...
  try {
    ...
    revalidateCacheEntries([appCacheTags.matches(serverId), appCacheTags.serverContext(serverId)]);
    return NextResponse.json(result);
  } catch (error) {
    const message = error instanceof Error && error.message ? error.message : "Unable to refresh performance history.";
