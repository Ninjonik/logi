...
export async function POST(request: Request) {
    ...
    try {
        const result = await fetchMutation(privacyRequest, {
            secret: getInternalAuthSecret(), userId: user.id, discordId: user.discordId,
            userName: user.name, type: body.type,
        })
        logNextInfo("privacy", "Privacy request created", { userId: user.id, type: body.type })
        return NextResponse.json({ ok: true, duplicate: result.duplicate })
    } catch (error) {
        logNextError("privacy", "Privacy request failed", { error, userId: user.id, type: body.type })
        return NextResponse.json({ error: "Unable to create the privacy request." }, { status: 500 })
    }
}
