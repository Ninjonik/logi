...
export async function POST(request: Request) {
    ...
    if (
        !body.primaryGuildId ||
        !body.secondaryGuildId ||
        body.primaryGuildId === body.secondaryGuildId
    )
        return NextResponse.json(
            { error: "Pick a primary and a different source clan." },
            { status: 400 }
        )
    try {
        await mergeCompetitionGuilds(body.primaryGuildId, body.secondaryGuildId)
