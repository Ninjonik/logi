...
const consumePlatformIdLinkTokenReference = makeFunctionReference<"mutation">(
...
export async function POST(
    ...
    if (!platformIds.length) {
        return NextResponse.json(
            { error: "Enter a valid platform ID." },
            { status: 400 }
        )
    }
    ...
    try {
        const result = (await fetchMutation(
            consumePlatformIdLinkTokenReference,
            {
        ...
        if (result.completionMode === "membership" && applicationMessageUrl) {
            const dmContent = formatTemplate(
                messages.membership.platformIdReadyDm,
                {
            ...
            try {
                ...
                if (
                    ...
                    result.interactionApplicationId
                ) {
                    try {
                        await editDiscordInteractionOriginalResponse({
                            ...
                            content: formatTemplate(
                                messages.membership.platformIdReadyInteraction,
                                {
                                    link: applicationMessageUrl,
                                }
                            ),
        ...
        return NextResponse.json({
            ok: true,
            message:
                result.completionMode === "membership"
