...
export default async function CommunityPage({ params, searchParams }: Props) {
    ...
    await connection()
    const [{ locale }, { q, matchesCursor, clansCursor, playersCursor }] =
    ...
    const [clans, matches, players] = await Promise.all([
        ...
        query.length >= 2
            ? searchPublicPlayers(query, playersCursor ?? null)
                ...
                <div className="space-y-10">
                    ...
                    <section className="bg-card rounded-2xl border p-5 sm:p-6">
                        <div className="max-w-xl">
                            <h2 className="text-xl font-semibold">
                                {dictionary.publicProfiles.findPlayer}
                            </h2>
                            ...
                            <PublicPlayerSearch
                                initialQuery={query}
                                label={dictionary.publicProfiles.findPlayer}
                                placeholder={
                                    dictionary.publicProfiles
                    ...
                    <section>
                        ...
                        {!clans.isDone ? (
                            <Link
                                className="text-primary mt-4 inline-block text-sm font-medium hover:underline"
                                href={`/${resolvedLocale}/community?${queryString({ clansCursor: clans.continueCursor })}`}
                            >
                                {dictionary.publicProfiles.loadMore}
                            </Link>
                        ) : null}
