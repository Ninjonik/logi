...
export default async function PublicPlayerPage({ params }: Props) {
  ...
  if (!player) notFound();
  const performanceHistory: PerformanceSnapshot[] = player.recentMatches.map((match) => ({
    ...
    {performanceHistory.length ? (
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-4">
          <CardTitle>{dictionary.clan.playerPerformanceTrend}</CardTitle>
          <PlayerTrendIndicators matches={performanceHistory} dictionary={dictionary} />
        </CardHeader>
      </Card>
    ) : null}
