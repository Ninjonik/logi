...
export default async function PublicPlayerPage({ params }: Props) {
  ...
  if (!player) notFound();
  const trends = getPerformanceTrendDeltas(player.recentMatches.map((match) => ({ kd: match.killDeathRatio, offense: match.offense + match.defense, support: match.support })));
  ...
  return <PublicSiteShell locale={resolvedLocale}><PublicPage><div className="space-y-6">
    ...
    {trends ? <Card><CardHeader><CardTitle>{dictionary.clan.playerPerformanceTrend}</CardTitle></CardHeader><CardContent className="flex flex-wrap gap-4 text-sm font-medium"><span className={trends.kd >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}>K/D {trends.kd >= 0 ? "↑ +" : "↓ "}{trends.kd}</span><span className={trends.offense >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}>{dictionary.clan.combatEffectiveness} {trends.offense >= 0 ? "↑ +" : "↓ "}{trends.offense}</span><span className={trends.support >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}>{dictionary.clan.supportEffectiveness} {trends.support >= 0 ? "↑ +" : "↓ "}{trends.support}</span></CardContent></Card> : null}
