export const enMessages = {
    publicProfiles: {
        communityTitle: "Community",
        communityDescription:
            "Browse active clans and their public match histories.",
        backToLogi: "Back to Logi",
        matches: "Matches",
        kills: "Kills",
        deaths: "Deaths",
        kd: "K/D",
        matchHistory: "Match history",
        matchPlayers: "Players",
        activeMembers: "active members",
        recentMatches: "Recent matches",
        wins: "Wins",
        winRate: "Win rate",
        clans: "Clans",
        findPlayer: "Find a player",
        findCommunity: "Find a player or clan",
        findPlayerDescription:
            "Search public profiles with at least one recorded match.",
        playerSearchPlaceholder: "Search by player name...",
        communitySearchPlaceholder: "Search players and clans...",
        search: "Search",
        noPlayersFound: "No public players matched that search.",
        noClansFound: "No public clans matched that search.",
        platformMatchHistoryDescription:
            "Results from every public clan, shown only after match data has been posted.",
        victory: "Victory",
        defeat: "Defeat",
        recorded: "Recorded",
        noMatches: "No public match results have been posted yet.",
        loadMore: "Load more",
    },
    app: {
        name: "Logi",
        tagline: "Hell Let Loose event organizer",
        description:
            "Organize clan events, build rosters, publish briefings, and prepare Discord-connected operations.",
    },
    games: {
        filterLabel: "Game filter",
        column: "Game",
        all: "All games",
        title: "Games",
        description: "Enable the games this clan uses.",
        save: "Save games",
        saved: "Enabled games saved.",
        saveError: "Unable to save enabled games.",
        selectTitle: "Choose a game first",
        selectDescription:
            "This record belongs to one game. Select the game before creating it.",
        noActiveTitle: "No active game",
        noActiveDescription:
            "Enable at least one game in clan settings before creating game-specific records.",
        openSettings: "Open clan settings",
    },
    publicNavigation: {
        wiki: "Wiki",
        restartTour: "Restart tour",
        privacy: "Privacy",
        gdpr: "GDPR",
        terms: "Terms",
    },
    home: {
        openApp: "Open app",
        signIn: "Sign in with Discord",
        contribute: "Contribute on GitHub",
        loggedInAs: "Signed in as {name}",
        dashboard: "Dashboard",
        community: "Explore community",
        competitions: "Competitions",
        featuresNav: "Features",
        liveOperations: "Operations, connected",
        commandCenter: "Your operation command center",
        commandCenterDescription:
            "Events, rosters, Discord coordination, and public match history in one place.",
        eventsMetric: "events",
        playersMetric: "players",
        matchesMetric: "matches",
        nextOperation: "Next operation",
        registrationOpen: "Registration open",
        sampleOperation: "Saturday competitive match",
        sampleOperationMeta: "19:30 · 31 registered",
        featuresEyebrow: "Built for organized teams",
        featureEventsDescription:
            "Plan every stage of an operation with clear times and Discord-ready announcements.",
        featureRostersDescription:
            "Build lineups, coordinate attendance, and keep everyone on the same page.",
        featureStats: "Public match history",
        featureStatsDescription:
            "Share player profiles, results, and detailed match analysis with your community.",
        badge: "Discord operations for organized groups",
        title: "Logi keeps community operations readable, structured, and in one place.",
        description:
            "Logi is an open-source platform for groups that run events, rosters, trainings, matches, and Discord coordination.",
        featuresTitle: "What it actually does",
        featuresDescription:
            "Plan events, manage signups, build rosters, prepare matches, and keep Discord synchronized.",
        featureEvents: "Events, trainings, and match workflows",
        featureRosters: "Roster and attendance management",
        featureDiscord: "Discord-linked server administration",
        featureTools: "Shared tooling for organized communities",
        commsBadge: "LogiComms",
        commsTitle: "Clear squad communication without the overhead.",
        commsDescription:
            "LogiComms is a lightweight, fast desktop app for efficient in-squad communication. Set it up quickly and tailor it to how your unit operates.",
        commsDownload: "Download for Windows",
        commsUnavailable: "Windows download is temporarily unavailable.",
        freeToUse: "Free to use",
        sourceAvailable: "Source available on GitHub",
        contributionsWelcome: "Contributions welcome",
        noMarketingFiller: "No marketing filler",
        landing: {
            heroEyebrow: "The operating system for your community",
            heroLineOne: "Every player.",
            heroLineAccent: "Every operation.",
            heroLineThree: "In formation.",
            heroDescription:
                "Logi turns the beautiful chaos of running a Hell Let Loose community into one living, breathing command center.",
            deployCommunity: "Deploy your community",
            seeEverything: "See everything it does",
            playersConfirmed: "31 players confirmed",
            marquee: [
                "Operations",
                "Rosters",
                "Matches",
                "Briefings",
                "Members",
                "Discord",
                "Analytics",
            ],
            arsenalEyebrow: "The complete arsenal",
            arsenalTitle:
                "Not a landing page full of promises. A real command center, screen by screen.",
            arsenalDescription:
                "Every module is already part of Logi. Explore the whole system below.",
            features: {
                dashboard: {
                    title: "Operations dashboard",
                    description:
                        "Your community’s pulse: events, performance, players and the next thing to run.",
                },
                calendar: {
                    title: "Calendar & events",
                    description:
                        "Plan trainings, matches and community nights without the Discord scavenger hunt.",
                },
                roster: {
                    title: "Roster builder",
                    description:
                        "Build squads visually, fill every role, and know exactly who is ready.",
                },
                briefing: {
                    title: "Briefings & maps",
                    description:
                        "Put the plan, the map and the orders where every player can find them.",
                },
                matches: {
                    title: "Match history",
                    description:
                        "Keep the record: results, scorelines, performance and moments that matter.",
                },
                members: {
                    title: "Member directory",
                    description:
                        "A real view of your people, roles, attendance and community health.",
                },
                event: {
                    title: "Event command",
                    description:
                        "From a simple signup to a full operational brief, every event gets its own home.",
                },
                presets: {
                    title: "Squad presets",
                    description:
                        "Set your unit structure once. Spin up a familiar formation in seconds.",
                },
                assignments: {
                    title: "Assignments",
                    description:
                        "Move players into the right place with a workflow built for commanders.",
                },
                matchData: {
                    title: "Match data",
                    description:
                        "Keep the detail behind every result close at hand for leaders, analysts, and players.",
                },
                api: {
                    title: "Public API & docs",
                    description:
                        "Extensive API reference and documentation for server admins, integrations, and contributors.",
                },
                wiki: {
                    title: "Community wiki",
                    description:
                        "Practical guides for every Logi feature, from your first workspace to daily operations.",
                },
            },
            workflowEyebrow: "From briefing to debrief",
            workflowTitle: "The work flows. So should your tools.",
            workflowDescription:
                "A proper operation is not one screen. It is a rhythm that Logi keeps together.",
            workflow: [
                {
                    title: "Plan",
                    description:
                        "Create an event, choose the map, publish the brief.",
                },
                {
                    title: "Fill",
                    description:
                        "Open signups, shape the roster, assign every slot.",
                },
                {
                    title: "Run",
                    description: "Give leaders a live, shared source of truth.",
                },
                {
                    title: "Remember",
                    description:
                        "Record results, follow performance, build the story.",
                },
            ],
            ownershipEyebrow: "Yours, forever",
            ownershipTitle: "Free to run. Free to own. Free to build on.",
            ownershipDescription:
                "Use Logi on our hosted platform at no cost, or run the full stack on your own infrastructure when data ownership is non-negotiable.",
            values: {
                hosted: {
                    title: "Hosted, free forever",
                    description:
                        "Start running your community immediately. Our hosted version is free — no feature gate, no surprise upgrade.",
                },
                selfHosted: {
                    title: "Self-host when it matters",
                    description:
                        "Want complete control over your data, environment, and deployment? Run Logi yourself and keep the keys.",
                },
                openSource: {
                    title: "Open source, deeply documented",
                    description:
                        "Explore the code, contribute improvements, and use extensive API documentation to connect Logi to your own tools.",
                },
            },
            finalTitle: "Your next operation is already waiting.",
            finalDescription:
                "Build the place your players trust before the first “ready check”.",
        },
        pillars: {
            operationsTitle: "Runs the boring parts",
            operationsDescription:
                "Scheduling, signups, roster organization, match prep, and Discord coordination in one place.",
            communitiesTitle: "Built for real communities",
            communitiesDescription:
                "Made for clans, units, and organized groups that need structure without extra ceremony.",
            contributorsTitle: "Open to contributors",
            contributorsDescription:
                "Open source, open to pull requests, and improved in public with people who use it.",
        },
    },
    auth: {
        loginTitle: "Deploy your next operation",
        loginDescription:
            "Sign in with Discord to manage clans, publish rosters, and keep every briefing in one place.",
        loginButton: "Continue with Discord",
        heroSubtitle: "Hell Let Loose Team Management made simple.",
        contributeButton: "Contribute",
        featureRosterManagement: "Roster management",
        featurePlayerPerformance: "Player performance aggregation",
        featureEventPlanning: "Event planning",
        featureOpenSource: "Open source",
        statsPlayers: "Players in database",
        statsMatches: "Matches tracked",
        statsTeams: "Teams using Logi",
        statsOperations: "Operations planned",
        loginHint:
            "Use Discord to open your clan workspace, rosters, and operations dashboard.",
        featureRostersTitle: "Rosters",
        featureRostersBody:
            "Publish slot-ready lineups with reserves and acknowledgements.",
        featureBriefingsTitle: "Briefings",
        featureBriefingsBody:
            "Keep event notes, maps, and attachments attached to every operation.",
        featureDiscordTitle: "Discord-ready",
        featureDiscordBody:
            "Prepare the web dashboard first, then plug in the bot layer later.",
        englishEnabled: "English enabled",
        discordAccessOnly: "Discord access only",
        discordOauth: "Discord OAuth",
        discordOauthBody: "Single sign-on for clans, members, and mercs.",
        plannedBackendHooks: "Planned backend hooks",
        plannedBackendHooksBody:
            "Convex documents, Discord identity sync, platform identity linking, clan-aware permissions, and roster publishing flows.",
    },
    dashboard: {
        title: "Your clans",
        description:
            "Everything you manage, represent, or fight alongside is grouped here.",
        greetingMorning: "Good morning",
        greetingAfternoon: "Good afternoon",
        greetingEvening: "Good evening",
        managedServers: "Managed clans",
        homeServer: "Main clan",
        mercenaryServers: "Mercenary clans",
        openServer: "Open clan",
        inviteBot: "Invite bot",
        inviteBotHint:
            "Some managed clans still need the Discord bot before they can open fully here.",
        inviteBotModalTitle: "Before you invite the bot",
        inviteBotModalDescription:
            "Put the bot role above every Discord role that Logi should assign or remove. If the bot role stays lower, Discord will block those role changes even if the permissions are enabled.",
        inviteBotModalNotApplicable:
            "This clan does not currently have any Discord roles configured for Logi to assign or remove, so the role-order warning does not apply yet.",
        inviteBotModalConfirm: "Open Discord invite",
        botStatus: "Bot status",
        botInstalled: "Installed",
        botMissing: "Missing",
        refreshBotStatus: "Refresh",
        refreshingBotStatus: "Refreshing bot status...",
        botStatusRefreshed: "Bot status refreshed.",
        botStatusRefreshError: "Unable to refresh bot status.",
        botTitle: "Bot operations",
        botDescription:
            "Global bot health, structured logs, and a filtered view across the dashboard app and Discord bot.",
        totalLogs: "Total logs",
        totalErrors: "Total errors",
        errorsToday: "Errors today",
        nextjsLogs: "Next.js logs",
        discordBotLogs: "Discord bot logs",
        topScopes: "Top scopes",
        filtersTitle: "Filters",
        filterLevel: "Level",
        filterSource: "Source",
        filterScope: "Scope",
        allLevels: "All levels",
        allSources: "All sources",
        allScopes: "All scopes",
        applyFilters: "Apply filters",
        resetFilters: "Reset filters",
        tableTimestamp: "Timestamp",
        tableLevel: "Level",
        tableSource: "Source",
        tableScope: "Scope",
        tableMessage: "Message",
        tableContext: "Context",
        noServerTitle: "No clans yet",
        noServerDescription:
            "Once a Discord account is connected, your clans will appear here.",
    },
    sidebar: {
        home: "Home",
        overview: "Overview",
        calendar: "Calendar",
        events: "Events",
        matches: "Matches",
        trainings: "Trainings",
        topicPresets: "Topic presets",
        stratmaps: "Stratmaps",
        groups: "Groups",
        squadPresets: "Squad presets",
        rosters: "Rosters",
        members: "Members",
        users: "Players",
        serverSettings: "Clan settings",
        system: "System",
        memberships: "Membership settings",
        tickets: "Tickets",
        userSettings: "User settings",
        serverLabel: "Active clan",
        workspace: "Workspace",
        operations: "Operations",
        configuration: "Configuration",
        bot: "Bot",
        competitions: "Competitions",
        articles: "Articles",
        logiComms: "LogiComms",
        signupActivity: "Signup activity",
    },
    signupActivity: {
        title: "Signup activity",
        description:
            "See who signed up, changed roles, or withdrew from matches and trainings.",
        empty: "No signup activity yet.",
        role: "Role",
        noRole: "No role",
        actions: {
            signed_up: "signed up",
            changed_role: "changed role",
            unsigned: "withdrew",
            declined: "declined",
        },
    },
    competition: {
        title: "Competitions",
        description:
            "Official competition information and results tracked in Logi.",
        back: "Back to Logi",
        standings: "View standings and results",
        rules: "Official rules",
        website: "Official website",
        seasonSummary:
            "{season} season · standings and results tracked by Logi",
        divisions: "{count} divisions · live standings and recorded results",
        results: "Results",
        statistics: "Match statistics",
        statisticsUnavailable: "Statistics unavailable",
        noCompetitions: "No public competitions have been created yet.",
        capScore: "Cap Score",
        regularWins: "Regular Wins",
        totalWins: "Total Wins",
        regularMatches: "Regular Matches",
        totalMatches: "Total Matches",
        team: "Team",
        withdrawn: "(withdrawn)",
        manageDescription:
            "Manage global seasons, teams, fixtures, and result links.",
        manualResult: "Add a manual result",
        manualResultDescription: "Use this when no Logi match event is linked.",
        division: "Division",
        teamA: "Team A",
        teamB: "Team B",
        chooseTeam: "Choose team",
        teamAScore: "Team A score",
        teamBScore: "Team B score",
        saveResult: "Save result",
        saving: "Saving…",
        resultSaved: "Result saved.",
        resultSaveFailed: "Unable to save the result.",
        mergeAliases: "Merge clan aliases",
        mergeDescription:
            "Move competition history from a ghost or alias clan to the real clan.",
        clanToKeep: "Clan to keep",
        chooseRealClan: "Choose the real clan",
        ghostToMerge: "Ghost or alias to merge",
        chooseDuplicate: "Choose the duplicate",
        mergeClans: "Merge clans",
        merging: "Merging…",
        mergeFailed: "Unable to merge clans.",
        mergeSuccess: "Clan history merged.",
        createEcl: "Create ECL 2026",
        createFailed: "Unable to create ECL.",
        createSuccess: "ECL 2026 created.",
    },
    articles: {
        title: "Articles",
        description: "Clan news and long-form updates.",
        createTitle: "Create article",
        createDescription: "Write a clan article in Markdown.",
        create: "Create article",
        noArticles: "No articles yet.",
        titlePlaceholder: "Title",
        descriptionPlaceholder: "Short description",
        tagsPlaceholder: "Tags, comma separated",
        bodyPlaceholder: "Article body",
        publish: "Publish article",
        attachments: "Attachments",
        saveFailed: "Unable to publish article.",
    },
    common: {
        actions: "Actions",
        edit: "Edit mode",
        cancel: "Cancel",
        save: "Save changes",
        create: "Create new",
        publish: "Publish",
        published: "Published",
        unpublished: "Draft",
        upcoming: "Upcoming",
        past: "Past",
        viewDetails: "View details",
        notAvailable: "Not available yet",
        assigned: "Assigned",
        reserves: "Reserves",
        acknowledge: "Acknowledge",
        acknowledged: "Acknowledged",
        membersOnly: "Members only",
        adminOnly: "Admins only",
        admins: "Admins",
        members: "Members",
        openAction: "Open",
        openStatus: "Open",
        settings: "Settings",
        logout: "Log out",
        account: "Account",
        today: "Today",
        searchReserves: "Search reserves...",
        searchNotAttending: "Search not attending...",
        noneAssigned: "None assigned.",
        unknown: "Unknown",
        createPreset: "Create preset",
        createEvent: "Create event",
        getPublicLink: "Get public share link",
        publicLinkCopied: "Public link copied.",
        publicLinkCopyFailed: "Unable to copy the public link.",
        createRoster: "Create roster",
        addSquad: "Add squad",
        addSlot: "Add slot",
        clear: "Clear",
        playerNote: "Player note",
        slotNote: "Note",
        otherGroups: "Other groups",
        openSlot: "Open slot",
        readyForAssignment: "Ready for assignment",
        saveAssignment: "Save assignment",
        removeAssignment: "Remove assignment",
        upload: "Upload",
        error: "Error",
    },
    emojiPicker: {
        pickEmoji: "Pick emoji",
        search: "Search emoji",
        clear: "Clear emoji",
        customCategory: "Server emojis",
        suggestedCategory: "Recently used",
        smileysPeopleCategory: "Smileys & people",
        animalsNatureCategory: "Animals & nature",
        foodDrinkCategory: "Food & drink",
        travelPlacesCategory: "Travel & places",
        activitiesCategory: "Activities",
        objectsCategory: "Objects",
        symbolsCategory: "Symbols",
        flagsCategory: "Flags",
    },
    userSettings: {
        privacyTitle: "Privacy and data",
        privacyDescription:
            "Request a copy of your personal data or request account erasure.",
        requestExport: "Request my data export",
        requestErasure: "Request account erasure",
        erasureWarning:
            "Erasure also removes your clan assignments, sign-ups, roster placements, and other account-linked records. Some data may be retained where legally required.",
        requestSubmitted: "Your privacy request has been submitted.",
        title: "User settings",
        description:
            "Manage your profile and platform identity used for imported match matching.",
        profile: "Profile",
        discordName: "Discord name",
        discordId: "Discord ID",
        avatar: "Avatar",
        preferredLanguage: "Preferred language",
        english: "English",
        notLinked: "Not linked",
        streamerMode: "Streamer mode",
        enabled: "Enabled",
        disabled: "Disabled",
        matchRecapsTitle: "Match recaps",
        matchRecapsEnabled: "Receive match recap messages",
        matchRecapsDescription:
            "Get a Discord recap after a recorded match when your roster slot is linked to its player stats.",
        defaultWorkspace: "Default workspace",
        defaultWorkspaceAutomatic: "Automatic (best available workspace)",
        defaultWorkspaceHelp:
            "This workspace opens when you enter the dashboard. Automatic prefers your primary clan, then another workspace you can access.",
        platformConnection: "Platform identity",
        platformConnected: "Platform ID set",
        platformDisconnected: "Platform ID not set",
        platformId: "Platform ID",
        platformIdPlaceholder: "Steam64 or Epic player ID",
        currentPlatformId: "Current platform ID",
        platformIdHelp:
            "Paste your Steam64 ID or Epic account ID here. No spaces.",
        platformIdSteamLink: "Steam guide",
        platformIdSteamHint:
            "Use Valve's guide to find your SteamID, then copy the long numeric Steam64 value.",
        platformIdEpicLink: "Epic guide",
        platformIdEpicHint:
            "Open Epic account settings or the launcher and copy your Account ID.",
    },
    serverSettings: {
        title: "Clan settings",
        description: "Manage how this clan looks and who can run operations.",
        pageDescription:
            "Update the clan profile, Discord automation settings, and access controls for your workspace.",
        frontendOnlyDescription: "Frontend-only clan configuration for now.",
        clanName: "Clan name",
        discordTitle: "Discord bot settings",
        discordDescription:
            "Control channels, roles, time zone handling, and group signup behavior for the Discord bot.",
        timezone: "Timezone",
        defaultLanguage: "Clan language",
        languageEnglish: "English",
        languageCzech: "Czech",
        languageGerman: "German",
        announcementsChannelId: "Announcements channel ID",
        eventInfoChannelId: "Event info channel ID",
        errorsChannelId: "Errors channel ID",
        calendarChannelId: "Calendar channel ID",
        forumCategoryId: "Forum category ID",
        meetingChannelId: "Meeting voice channel ID",
        clanRoleId: "Clan role ID",
        dashboardAdminRoleId: "Dashboard admin role ID",
        playerStatsServersTitle: "Server stats connections",
        playerStatsServersDescription:
            "Add one or more stats API connections. Player search will query all configured servers.",
        playerStatsServerToken: "Server stats token",
        playerStatsServerTokenPlaceholder: "bearer ...",
        playerStatsServerUrl: "Server stats URL",
        playerStatsServerUrlPlaceholder: "https://.../api/get_players_history",
        addPlayerStatsServer: "Add stats server",
        removePlayerStatsServer: "Remove stats server",
        rosterScoreTitle: "Roster score rules",
        rosterScoreDescription:
            "Adjust how player score changes once event signups are finalized.",
        rosterScoreNoResponse: "No reaction (-2)",
        rosterScoreDeclined: "Declined (-1)",
        rosterScoreAccepted: "Any other reaction (+1)",
        guildLoginUrl: "Custom login page",
        copyLoginUrl: "Copy",
        copiedLoginUrl: "Copied",
        eventCategoriesTitle: "Event categories",
        eventCategoriesDescription:
            "Reusable categories for matches and trainings. Their color and emoji are reused across dashboard calendar surfaces.",
        addEventCategory: "Add category",
        eventCategoryLabel: "Category",
        eventCategoryName: "Category name",
        noEventCategories: "No event categories yet.",
        calendarItemsTitle: "Calendar items",
        calendarItemsDescription:
            "Add manual calendar-only entries such as summer breaks, clan meetings, or other recurring reminders.",
        addCalendarItem: "Add calendar item",
        calendarItemLabel: "Calendar item",
        calendarItemTitle: "Title",
        calendarItemLabelName: "Legend label",
        calendarItemAllDay: "All day",
        calendarItemStart: "Starts",
        calendarItemEnd: "Ends",
        calendarItemRecurrence: "Recurrence",
        calendarItemRecurrenceInterval: "Repeat every",
        calendarItemRecurrenceUntil: "Repeat until",
        recurrenceNone: "Does not repeat",
        recurrenceWeekly: "Weekly",
        recurrenceMonthlyDate: "Monthly by date",
        recurrenceMonthlyNthWeekday: "Monthly by weekday",
        recurrenceYearly: "Yearly",
        noCalendarItems: "No manual calendar items yet.",
        groupRoleMappings: "Group role mappings",
        requiredRoleId: "Required role ID",
        groupEmoji: "Group emoji",
        noGroupRole: "No required role",
        saveDiscordSettings: "Save Discord settings",
        discordSettingsSaved: "Discord settings saved.",
        discordSettingsSaveError: "Unable to save Discord settings.",
        botRequiredNotice:
            "The bot must already be in the server before the full workspace becomes available.",
        accessModel: "Access model",
        accessAdmins:
            "Admins: full access to events, presets, rosters, and clan settings.",
        accessMembers:
            "Members and mercenaries: calendar visibility plus published rosters only.",
        accessBackend:
            "Later backend work can connect these rules to Discord roles and Convex queries.",
    },
    ticketSettings: {
        title: "Ticket settings",
        pageDescription:
            "Configure the ticket panel, categories, staff roles, and modal questions for Discord tickets.",
        enableTitle: "Enable ticket system",
        enableDescription:
            "Post a configurable ticket panel in Discord and open private ticket threads from its category buttons.",
        submitChannel: "Submit ticket channel",
        parentChannel: "Ticket thread parent channel",
        panelTitle: "Panel embed title",
        panelDescription: "Panel embed description",
        panelDescriptionPlaceholder:
            "Explain how members should use this ticket panel.",
        image: "Thumbnail image",
        embedLimitNotice:
            "The category explanation field in the embed is currently using",
        embedLimitExceeded:
            "(too long, trim some category descriptions before saving).",
        categoriesTitle: "Ticket categories",
        categoriesDescription:
            "Each category becomes a button and can optionally open a Discord modal before the ticket thread is created.",
        addCategory: "Add category",
        categoryLabel: "Category",
        buttonText: "Button text",
        buttonTextPlaceholder: "Report a player",
        emoji: "Emoji",
        pickServerEmoji: "Pick a server emoji",
        typeAnyEmoji: "or type any emoji",
        categoryDescription: "Description",
        categoryDescriptionPlaceholder:
            "Shown in the embed field to explain what this category is for.",
        supportRoles: "Support roles invited to this ticket",
        modalQuestions: "Modal questions",
        modalQuestionsDescription:
            "Discord supports up to 5 text inputs per modal.",
        addQuestion: "Add question",
        questionLabel: "Question",
        questionText: "Question label",
        questionTextPlaceholder: "What happened?",
        inputStyle: "Input style",
        shortInput: "Short",
        paragraphInput: "Paragraph",
        placeholder: "Placeholder",
        required: "Required",
        noQuestions:
            "No modal questions yet. If you leave this empty, clicking the button will create the ticket immediately.",
        noCategories: "No ticket categories yet.",
        defaultPanelTitle: "Submit a ticket",
        defaultPanelDescription:
            "Pick the category that fits your issue best and we will open a private support thread for you.",
        incompleteTitle: "Ticket panel setup is incomplete",
        incompleteDescription:
            "The ticket system is enabled, but the Discord ticket panel will not be posted and ticket threads cannot open until you finish: {items}.",
        routingInfoTitle: "How ticket routing works",
        routingInfoDescription:
            "Category support roles are invited into new ticket threads automatically. Users with the dashboard admin role are also treated as support staff for ticket handling.",
    },
    membershipSettings: {
        title: "Membership settings",
        pageDescription:
            "Configure the Discord application embed, required application categories, and default assignment behavior for new clan members.",
        enableTitle: "Enable membership applications",
        enableDescription:
            "Post a clan application embed in Discord and open staff-managed private threads from it.",
        submitChannel: "Submit embed channel",
        parentChannel: "Application thread parent",
        skipPendingTitle: "Skip pending phase",
        skipPendingDescription:
            "New member applications start as recruits immediately after submission.",
        panelTitle: "Panel title",
        panelDescription: "Panel description",
        image: "Thumbnail image",
        applicationThumbnail: "Application thumbnail",
        categoriesTitle: "Application categories",
        categoriesDescription:
            "Each category becomes a button. Modal questions are optional and reuse the same pattern as tickets.",
        addCategory: "Add category",
        buttonLabel: "Button label",
        applicationResult: "Application result",
        recruitRole: "Recruit role for this category",
        recruitRolePlaceholder: "Recruit role",
        finalRole: "Final role for this category",
        finalRolePlaceholder: "Final role",
        modalQuestionsDescription:
            "Up to 5 text inputs, just like the ticket modal flow.",
        noQuestions:
            "No modal questions yet. Leave this empty if the category should open the thread immediately after the precheck.",
        noCategories: "No application categories yet.",
        defaultPanelTitle: "Apply to the clan",
        defaultPanelDescription:
            "Pick the application type that matches you. If we still need your platform ID, we will guide you through it first.",
        embedFieldUsage: "Embed field usage: {length} / {max}",
        embedFieldTooLong:
            "(too long, trim category descriptions before saving)",
        incompleteTitle: "Membership panel setup is incomplete",
        incompleteDescription:
            "Applications are enabled, but the Discord membership panel will not be posted and application threads cannot open until you set {items}.",
        roleSyncTitle: "How membership role sync works",
        roleSyncDescription:
            "Pending applications do not assign membership roles. Recruit status uses the clan role plus the category recruit role, and active status uses the clan role plus the category final role when those roles are configured.",
        rosterScoreDescription:
            "Set per-server score changes after the event concludes. Leaving everything at 0 disables automatic score movement.",
        rosterScoreNoCategory: "No category / no reaction",
        rosterScorePresentRoster: "Reacted and present in roster",
        rosterScorePresentReserve: "Reacted and present in reserves",
        rosterScoreAbsentRoster: "Reacted and absent from roster",
        rosterScoreAbsentReserve: "Reacted and absent from reserves",
        rosterScoreExcusedAbsence: "Reacted, absent, but had notice",
        rolesMissingTitle: "Some membership roles are not configured",
        rolesMissingClanRole: "The base clan role is not set. ",
        rolesMissingRecruitRole:
            "{count} member {noun} {verb} missing a recruit role. ",
        rolesMissingFinalRole: "{count} {noun} {verb} missing a final role. ",
        rolesMissingSummary:
            "In those cases, Logi will still track the application state, but Discord role changes will be partial or skipped.",
        singleCategory: "category",
        multipleCategories: "categories",
        singleIs: "is",
        pluralAre: "are",
        save: "Save membership settings",
        saved: "Membership settings saved.",
        saveError: "Unable to save membership settings.",
    },
    userManagement: {
        grantAdminAccess: "Make admin",
        removeAdminAccess: "Remove admin access",
        adminAccessGranted: "Player is now an admin.",
        adminAccessRemoved: "Player admin access removed.",
        adminAccessUpdateFailed: "Unable to update player admin access.",
        title: "Players",
        description:
            "Add players from the existing system as main members or mercenaries while enforcing clan membership rules.",
        addMember: "Add as member",
        addReserveMember: "Add as reserve member",
        addMerc: "Add as mercenary",
        removeMember: "Remove member",
        removeMerc: "Remove mercenary",
        memberLabel: "Main member",
        reserveMemberLabel: "Reserve member",
        mercLabel: "Mercenary",
        eligibleMembers: "Eligible for member",
        eligibleMercs: "Eligible for mercenary",
        blocked: "Not eligible",
        searchPlaceholder: "Search players in the system...",
        noResults: "No matching players found in the system.",
        rulesTitle: "Assignment rules",
        rulesMember:
            "A player can only become a main member if they do not already belong to another main clan.",
        rulesMerc:
            "A player can be a mercenary for other clans, but cannot be both member and mercenary in the same clan.",
        rulesSystem:
            "Search only returns players already present in our system, ready for backend autocomplete later.",
        tablePlayer: "Player",
        tableType: "Type",
        tableGroup: "Group",
        tableStatus: "Status",
        tableScore: "Score",
        platformId: "Platform ID",
        playerNote: "Player note",
        platformNotConnected: "Platform ID not set",
        platformConnectedAs: "Platform ID set as {platformId}",
        platformIdNoSpaces: "Platform ID cannot contain spaces.",
        platformAlreadyLinked:
            "This platform ID is already linked to another player.",
        paused: "Paused",
        active: "Active",
        membershipStatus: "Membership status",
        pendingLabel: "Pending",
        recruitLabel: "Recruit",
        addPlayer: "Add player",
        autoLinkPlatformIds: "Auto-link Platform IDs",
        autoLinkPlatformIdsDescription:
            "Scan imported event scoreboards, strip your clan tag from player names, and fill in matching platform IDs automatically.",
        autoLinkPlatformIdsHint:
            "Enter the clan tag exactly as it appears before player names in imported events. Example: VLKㆍ",
        autoLinkPlatformIdsSuccess:
            "Linked {linked} platform IDs from {players} matched player entries.",
        autoLinkPlatformIdsPartial:
            "Skipped some results: {ambiguous} ambiguous, {conflicts} conflicts, {failed} failed events.",
        dedupePlayerStats: "Dedupe player stats",
        dedupePlayerStatsDescription:
            "Remove duplicate imported match rows for this clan's events and keep the better non-Unknown copy when possible.",
        dedupePlayerStatsHint:
            "This only cleans imported player stats for events that belong to this clan. It does not touch other clans' matches.",
        dedupePlayerStatsSuccess:
            "Removed {matches} duplicate match rows across {users} players.",
        refreshPerformanceHistory: "Refresh clan & player stats",
        refreshPerformanceHistorySuccess: "Clan and player stats refreshed.",
        refreshPerformanceHistoryError:
            "Unable to refresh clan and player stats.",
        mergeUsers: "Merge users",
        mergeUsersDescription:
            "Merge two accounts completely. The primary account wins whenever both accounts contain overlapping data.",
        mergeUsersPrimary: "Primary account",
        mergeUsersSecondary: "Secondary account",
        mergeUsersPickPrimary: "Pick the account to keep",
        mergeUsersPickSecondary: "Pick the account to merge into it",
        mergeUsersHint:
            "This rewrites player stats, imported matches, assignments, rosters, and embedded event references to the primary account.",
        mergeUsersConfirm: "Merge accounts",
        mergeUsersSuccess: "Accounts merged.",
        clanTag: "Clan tag",
        clanTagPlaceholder: "VLKㆍ",
        importDiscordMembers: "Import Discord members",
        importDiscordMembersDescription:
            "Import everyone who has a chosen Discord role into player management.",
        importDiscordMembersHint:
            "Imported players get no primary group. Matching Discord-required group roles are added as secondary groups.",
        membershipMigration: "Migrate membership by role",
        membershipMigrationDescription:
            "Pick a Discord role and the membership state it should map to for existing clan assignments.",
        membershipMigrationTarget: "Target membership state",
        membershipMigrationHint:
            "Only players already assigned in this clan are updated. Unassigned Discord members are skipped.",
        membershipMigrationSuccess: "Updated {updated} player(s) to {status}.",
        membershipMigrationSkippedUnassigned:
            "Skipped {count} Discord member(s) without a clan assignment.",
        membershipMigrationSkippedUnchanged:
            "Skipped {count} player(s) already in that membership state.",
        linkMissingDiscordIds: "Link missing Discord IDs",
        linkMissingDiscordIdsDescription:
            "Match imported players without Discord IDs against members of a selected Discord role.",
        linkMissingDiscordIdsHint:
            "Names are normalized before matching and the first matching Discord member is linked.",
        linkMissingDiscordIdsSuccess:
            "Linked {linked} imported player(s) from {scanned} candidate(s).",
        linkMissingDiscordIdsMerged:
            "Merged {count} imported player record(s) into existing Discord-linked users.",
        discordSourceRole: "Discord source role",
        discordImportSuccess:
            "Imported {players} players from {matched} matching Discord members.",
        discordImportSkipped:
            "{count} matching Discord member(s) were skipped because they could not be imported with the selected assignment type.",
        editAssignment: "Edit assignment",
        playerSearch: "Player search",
        assignmentType: "Assignment type",
        primaryGroup: "Primary group",
        secondaryGroups: "Secondary groups",
        noGroup: "No primary group",
        noSecondaryGroups: "No secondary groups",
        pauseMembership: "Pause membership",
        pauseHelp: "Use this for inactivity or temporary absence.",
        pauseNote: "Pause note",
        pauseNoteRequired: "Add a pause note when membership is paused.",
        pickPlayerFirst: "Pick a player from the in-system search list first.",
        currentAssignment: "Current assignment",
        mainClan: "Main clan",
        none: "None",
        assignmentDescription:
            "Edit assignment type, group, paused membership state, or remove the player from this clan.",
        assignmentMetaDescription:
            "Edit clan membership, mercenary role, group, and pause state.",
        assignmentTitleSuffix: "assignment",
        averageDescription: "Average across the last {count} matches.",
        storedAverageDescription: "Average across {count} imported matches.",
        averageKills: "Avg kills",
        averageKd: "Avg K/D",
        averageDeaths: "Avg deaths",
        averageOffense: "Avg offense",
        averageDefense: "Avg defense",
        averageSupport: "Avg support",
        matchKills: "Kills",
        matchKd: "K/D",
        matchDeaths: "Deaths",
        matchOffense: "Offense",
        matchDefense: "Defense",
        matchSupport: "Support",
        matchHistorySearch: "Search match history...",
        matchHistoryEvent: "Event",
        matchHistoryPlayedAt: "Played",
        removePlayerFromClan: "Remove the player from this clan.",
        removePlayerConfirm:
            "Remove this player from the clan? This will delete their assignment.",
        saveError: "Unable to save the player assignment.",
        deleteError: "Unable to delete the player assignment.",
        assignmentSaved: "Player assignment saved.",
        assignmentCreated: "Player assignment created.",
        assignmentDeleted: "Player assignment deleted.",
        primaryGroupRequired: "Pick a primary group before saving.",
        alreadyAssigned: "This player is already assigned to this clan.",
        mercenaryRecruitStatusError:
            "Mercenaries cannot use the recruit status.",
        reserveMemberRecruitStatusError:
            "Reserve members cannot use the recruit status.",
        roleSyncTitle: "How membership role sync works",
        roleSyncDescription:
            "Group-linked Discord roles sync from the selected primary and secondary groups. Membership roles only sync for non-pending members, and recruit or active status depends on the clan role plus the linked membership category roles when available.",
        missingClanRoleTitle: "Clan role is not configured",
        missingClanRoleDescription:
            "This player can still be marked as recruit or active in Logi, but no base clan role will be added in Discord until the clan role is set in clan settings.",
        missingMembershipCategoryTitle:
            "This assignment is not linked to a membership category",
        missingMembershipCategoryDescription:
            "Recruit and final category roles cannot be synced for manual assignments without a linked membership application category. Only the clan role and group-linked roles can be applied in Discord.",
        missingCategoryRolesTitle: "Membership category roles are incomplete",
        missingRecruitRoleDescription:
            "This category has no recruit role configured. ",
        missingFinalRoleDescription:
            "This category has no final role configured. ",
        missingCategoryRolesSummary:
            "Logi will keep the membership status here, but the matching Discord role will not be added for that stage.",
    },
    navUser: {
        scoreSuffix: "score",
    },
    workspace: {
        selectWorkspace: "Select workspace",
        activeWorkspace: "Active workspace",
        noWorkspaceSelected: "No workspace selected",
        searchWorkspace: "Search workspaces...",
    },
    languageSwitcher: {
        selectLanguage: "Select language",
    },
    platformIdLink: {
        title: "Link your platform ID",
        description:
            "Pick your platform first. We will then show you exactly what to copy and where to find it.",
        userPrefix: "Submitting for",
        expired:
            "This link is no longer valid. Go back to Discord and click the clan application button again to get a fresh link.",
        platformLabel: "Platform",
        platformPlaceholder: "Select your platform",
        submit: "Submit platform ID",
        success:
            "Platform ID saved. You can close this page now and return to Discord.",
        closePage: "You can close this page now.",
        genericError: "Unable to save platform ID.",
        guideLabel: "Open guide",
        steam: {
            label: "Steam",
            idLabel: "Steam64 ID",
            placeholder: "7656119...",
            guideLabel: "Open Steam guide",
            help: "You need the long Steam number for your account.",
            steps: [
                "Open the guide below.",
                "Follow the pictures until you see your Steam64 ID.",
                "Copy that long number and paste it here.",
            ],
        },
        epic: {
            label: "Epic Games",
            idLabel: "Epic Account ID",
            placeholder: "Epic account ID",
            guideLabel: "Open Epic guide",
            help: "You need your Epic Account ID.",
            steps: [
                "Open the guide below.",
                "Open your Epic account page.",
                "Copy the Account ID it shows and paste it here.",
            ],
        },
        xbox: {
            label: "Xbox",
            idLabel: "Xbox gamertag / account ID",
            placeholder: "Xbox gamertag or account ID",
            guideLabel: "Open Xbox guide",
            help: "Use the Xbox identity you play Hell Let Loose with. In most cases this is your gamertag.",
            steps: [
                "Open the guide below.",
                "Open your Xbox profile.",
                "Copy the gamertag or account value you use in game and paste it here.",
            ],
        },
        playstation: {
            label: "PlayStation",
            idLabel: "PlayStation online ID",
            placeholder: "PlayStation online ID",
            guideLabel: "Open PlayStation guide",
            help: "You need your PlayStation online ID.",
            steps: [
                "Open the guide below.",
                "Open your PlayStation profile settings.",
                "Copy your online ID and paste it here.",
            ],
        },
    },
    calendarPage: {
        title: "Calendar",
        description:
            "Use the calendar to browse upcoming and past events, then jump into full event details or the published roster.",
        monthView: "Monthly operations view",
        moreEvents: "more events",
        allDay: "All day",
    },
    calendarCards: {
        eventCalendar: "Event calendar",
        registrationEnds: "Registration ends",
        meeting: "Meeting",
        gameStart: "Game start",
        map: "Map",
        showRoster: "Show roster",
        noEvents:
            "No events are scheduled for the selected date in this preview.",
    },
    roster: {
        title: "Roster",
        listDescription:
            "Browse event rosters, check squad counts, and see which lineups are already published.",
        modeView: "View only",
        modeLayout: "Squad editor",
        modeAssignment: "Roster editor",
        updatePublished: "Update published roster",
        publishRoster: "Publish roster",
        unpublishRoster: "Unpublish roster",
        deleteRoster: "Delete roster",
        deleteRosterTitle: "Delete draft roster?",
        deleteRosterDescription:
            "This permanently deletes this unpublished roster. Published rosters must be unpublished first.",
        deleted: "Roster deleted",
        changeSquadTemplateTitle: "Change squad template?",
        changeSquadTemplateDescription:
            "This replaces the squad layout and resets all manual player assignments. Currently assigned players will move back to reserves.",
        changeSquadTemplateAction: "Change template and reset assignments",
        squadTemplateChanged:
            "Squad template changed. Save the roster to apply it.",
        publishConfirmTitle: "Publish roster?",
        publishConfirmDescription:
            "Members will see the published version immediately.",
        setupRoster: "Setup Roster",
        selectEvent: "Select Event",
        selectEventPlaceholder: "Choose an event",
        selectPreset: "Select Squad Preset",
        selectPresetPlaceholder: "Choose a preset",
        noEventsAvailable: "No events without rosters available",
        selectEventAndPresetToStart:
            "Select an event and a squad preset to start drafting the roster",
        matchTime: "Match time",
        opponent: "Opponent",
        mapSide: "Map & side",
        notes: "Notes",
        noExtraNotes: "No extra notes yet.",
        squadSetup: "Squad setup",
        role: "Role",
        moveToReserves: "Move to reserves",
        moveToNotAttending: "Move to not attending",
        addSlot: "Add slot",
        addSquad: "Add squad",
        newSquadName: "Squad",
        newRoleName: "New role",
        defaultSquadGroup: "Infantry Squad",
        defaultSquadLeadRole: "Squad Lead",
        versusDelimiter: "vs",
        rosterNotAssigned: "Roster has not been assigned to an event yet.",
        rosterNotCreated: "Roster has not been created yet.",
        rosterNotAvailable: "Roster not yet available.",
        notAttending: "Not attending",
        declinedSignup: "Explicitly declined signup",
        noSignupResponse: "No signup response",
        unsignedPlayerConfirmTitle: "Add unsigned player?",
        unsignedPlayerConfirmDescription:
            "{name} has not signed up for this event. Are you sure you want to add them to the roster?",
        unsignedPlayerConfirmAction: "Add to roster",
        autoFill: "Auto-fill roster",
        autoFillDescription:
            "Players are assigned strongest-first, then placed into the closest available squad by signup preference, primary group, and secondary groups.",
        autoFillScoreWeight: "Score weight",
        autoFillKdWeight: "K/D weight",
        autoFillConfirm: "Auto-fill now",
        autoFilled: "Roster auto-filled",
        confirmFromMeetingChannel: "Confirm from meeting VC",
        confirmFromMeetingChannelHelp:
            "Set a meeting voice channel in clan settings to enable this action.",
        confirmingFromMeetingChannel: "Confirming from meeting VC...",
        confirmedFromMeetingChannel:
            "Confirmed {count} rostered player(s) from the meeting VC.",
        noRosterPlayersInMeetingChannel:
            "No rostered players were found in the meeting VC.",
        noNewMeetingChannelAttendanceChanges:
            "Rostered player(s) were found in the meeting VC, but no new attendance changes were needed.",
        attendancePending: "Not acknowledged",
        attendanceAcknowledged: "Acknowledged",
        attendanceConfirmed: "Confirmed",
        saved: "Roster saved",
        published: "Roster published",
        attendanceUpdated: "Attendance updated",
        updatePublishedPromptTitle: "Post a roster update too?",
        updatePublishedPromptDescription:
            "This roster is already live. Pick whether to send a fresh football-style update into the same announcements channel while the affected players are DM'd either way.",
        updatePublishedPromptAnnounce: "Save and post update",
        updatePublishedPromptSkip: "Save without post",
        updatePublishedPromptCancel: "Keep editing",
        updatePublishedPromptHint:
            "⚽ We can call out who got in, who got dropped, squad moves, and role changes.",
        notifyRosterChanges: "Notify affected players by DM",
        postRosterChanges: "Post this roster update in the event info channel",
        updatePosted: "Roster saved and update posted.",
        updateSavedWithoutPost: "Roster saved. Player DMs were sent.",
        updateDmDeliveryFailed:
            "Roster saved, but one or more player DMs could not be delivered.",
    },
    event: {
        infoTitle: "Event information",
        infoDescription:
            "This page stays view-first for everyone, and admins can toggle it into edit mode without changing routes.",
        listDescription:
            "Track planned operations, meeting times, map details, and publication settings in one place.",
        createTitle: "Create event",
        created: "Event created.",
        saved: "Event saved.",
        createDescription:
            "Set up the event details, schedule, and Discord options before publishing it to your clan.",
        createPageDescription:
            "Create a new event with the same fields you can manage later from the event detail page.",
        relatedRoster: "Related roster",
        showRoster: "Show roster",
        rosterPublishedHint:
            "This event has a published roster. Open it from here when needed.",
        rosterDraftHint:
            "A roster is assigned, but it is still hidden from regular members.",
        rosterMissingHint:
            "No roster is assigned yet. Add one later when you are ready to organize squads for this event.",
        detailMetaTitle: "Event details",
        requiredLabel: "Required",
        optionalLabel: "Optional",
        noPreset: "No preset",
        topicPreset: "Topic preset",
        topicPresetCompleteMatch: "Complete match",
        topicPresetPartialMatch: "Partial match",
        resyncTopicThread: "Resync topic thread",
        topicThreadResyncQueued: "Topic thread resync queued.",
        generalSignupDescription:
            "Enable one general signup button that assigns players to their primary group when it is available for this match, otherwise to reserves.",
        signupGroupsDescription:
            "Choose which group-specific signup buttons should be shown for this match.",
        allowedSignupStatusesDescription:
            "Leave this empty to allow any clan member status to sign up. Otherwise only the selected statuses can sign up for this match.",
        allowedSignupStatusesAll: "All clan member statuses",
        signupReminderStatusesDescription:
            "Send the full match signup message by DM every 24 hours, starting 24 hours after creation and stopping at registration close. Select no statuses to disable it.",
        signupReminderDisabled: "Daily signup reminders are disabled.",
        createForumChannelDescription:
            "Create the Discord forum channel and briefing topics for this event.",
        conclude: "Conclude event",
        submitMatchResults: "Submit match results",
        submitMatchResultsDescription:
            "Paste a match link that ends with /games/[id]. The app will import the scoreboard and save player stats.",
        importEvents: "Import events",
        importEventsDescription:
            "Paste event links in the same format as match result imports. Commas and line breaks are both supported.",
        importEventsHint: "{count} link(s) ready to import.",
        importEventsSuccess:
            "Imported {events} event(s) and {players} player records.",
        importEventsPartial: "{failed} link(s) could not be imported.",
        importEventPlayers: "Import and match players",
        importEventPlayersDescription:
            "Match imported HLL players against the database, link missing platform IDs, and create new players when needed.",
        importEventPlayersHint:
            "Player names are compared after stripping the clan tag, lowercasing, and removing special characters.",
        matchLink: "Match link",
        matchLinks: "Event links",
        matchResultsImported:
            "Imported {players} player records. Event result: {result}.",
        resultSaved: "saved",
        resultSkipped: "skipped",
        resultVictory: "Victory",
        resultDefeat: "Defeat",
        resultDraw: "Draw",
        resultColumn: "Result",
        matchColumn: "Match",
        openMatch: "Open match",
        matchDetailTitle: "Match details",
        matchDetailDescription:
            "Raw imported scoreboard, player performance, and team breakdowns for this event.",
        rawSource: "Source",
        importedAt: "Imported",
        playedAt: "Played",
        duration: "Duration",
        players: "Players",
        score: "Score",
        overviewTab: "Overview",
        playersTab: "Players",
        weaponsTab: "Weapons",
        killsByTypeTab: "Kills by type",
        noMatchLinked: "No imported match is linked to this event yet.",
        alliedTeam: "Allies",
        axisTeam: "Axis",
        teamUnknown: "Unknown",
        topWeapons: "Top weapons",
        deathsByWeapon: "Deaths by weapon",
        playerBreakdowns: "Player breakdowns",
        standoutAwards: "Standout awards",
        topRivalries: "Top rivalries",
        encounters: "Encounters",
        encountersDescription:
            "Derived from imported per-player aggregates such as most killed and death by.",
        killsByWeaponForPlayer: "Kills by weapon",
        deathsByWeaponForPlayer: "Deaths by weapon",
        badges: "Badges",
        killed: "Killed",
        diedTo: "Died to",
        plusMinus: "+/-",
        noDerivedData: "No derived data",
        performanceScatter: "Kills vs deaths",
        rawStats: "Raw stats",
        concludedSuccess: "Event concluded",
        trainingCompletionTitle: "Complete training",
        trainingCompletionDescription:
            "Set a pass or fail result for each attending participant before concluding the training.",
        trainingCompletionEmpty:
            "No attending participants are currently signed up for this training.",
        trainingCompletionSuccess:
            "Training completed. Rewarded: {rewarded}. DMs sent: {dmed}.",
        trainingPassed: "Passed",
        trainingFailed: "Failed",
        trainingCompleteAction: "Save and conclude",
        saveHelp:
            "Name and the four timeline fields are required. Everything else is optional.",
        saveError: "Unable to save event.",
        notices: {
            announcementsTitle: "Announcements channel is not set",
            announcementsDescription:
                "Event announcement messages will not be posted or updated in Discord until an announcements channel is configured for this clan.",
            forumTitle: "Forum category is not set",
            forumDescription:
                "This event is set to create a Discord forum channel, but no forum category is configured. The forum channel and briefing topics will be skipped until that setting is filled in.",
            meetingAutomationTitle: "Meeting voice automation is disabled",
            meetingAutomationDescription:
                "Discord scheduled events and roster attendance confirmation from the meeting VC both depend on the clan meeting voice channel setting.",
            openClanSettings: "Open clan settings",
        },
        statuses: {
            registration: "Registration",
            closed: "Closed",
            starting: "Starting",
            concluded: "Concluded",
        },
        fields: {
            name: "Name",
            kind: "Type",
            matchType: "Event category",
            description: "Description",
            thumbnail: "Thumbnail",
            image: "Embed image",
            announcementChannelId: "Event registration channel",
            eventInfoChannelId: "Event info channel",
            meetingChannelId: "Meeting VC / location",
            server: "Server",
            serverPassword: "Server password",
            map: "Map",
            mapVariant: "Time of day / weather",
            mapMode: "Gamemode",
            mapPresetCode: "Map preset code",
            side: "Side",
            capMode: "Middle Cap Point",
            registrationEnd: "Registration end",
            meetingStart: "Meeting start",
            gameStart: "Game start",
            gameEnd: "Game end",
            notes: "Notes",
            stratmaps: "Stratmaps",
            requiredRoleIds: "Required role IDs",
            rewardRoleIds: "Reward role IDs",
            signupGroupIds: "Visible signup groups",
            allowedSignupStatuses: "Allowed signup statuses",
            signupReminderStatuses: "Daily signup reminder recipients",
            useGeneralSignup: "Enable general signup",
            pingClan: "Ping clan",
            pingMode: "Announcement ping",
            pingRoleIds: "Roles to ping",
            createForumChannel: "Create forum channel",
        },
        channelRoutingCreateHelp:
            "Starts with the clan settings as a preset. This choice is locked after creation to keep Discord messages stable.",
        channelRoutingSharedHelp:
            "Choose the registration channel. If the event info channel is left empty, the registration channel is used for both messages.",
        channelRoutingLockedHelp:
            "This routing is locked after creation to keep Discord message IDs stable.",
        pingNone: "Do not ping anyone",
        pingClanOption: "Ping the clan role",
        pingRolesOption: "Ping selected roles",
        quickScheduleTitle: "Quick schedule",
        quickScheduleDescription:
            "Set the four key times for this match. You can revise the generated timeline at any time.",
        registrationHoursBefore: "Hours before headcount to close registration",
        meetingMinutesBefore: "Minutes before start for the meeting",
        durationHours: "Duration (hours)",
        durationMinutes: "Match duration (minutes)",
        quickEventStart: "Match start date and time",
        applyQuickSchedule: "Apply schedule",
        quickScheduleApplied: "Schedule applied.",
        recurringMatch: "Make this a recurring match",
        recurringMatches: "Recurring matches",
        recurringMatchesDescription:
            "Manage the match series that repeat on a schedule.",
        noRecurringMatches: "No recurring matches have been created yet.",
        recurrenceHelp:
            "The match being created is the first match in this series. Its details and time are used for future occurrences.",
        recurrenceFrequency: "Repeat",
        recurrenceWeekly: "Weekly",
        recurrenceMonthlyDate: "Monthly on a date",
        recurrenceMonthlyNthWeekday: "Monthly on a weekday",
        recurrenceInterval: "Every (interval)",
        recurrenceWeekdays: "Days of the week",
        recurrenceMonthDay: "Day of month",
        recurrenceNth: "Week of month",
        recurrenceWeekday: "Weekday",
        editSchedule: "Edit schedule",
        quickScheduleStepLabel: "Step {step} of 4",
        wizardBack: "Back",
        wizardNext: "Next",
        discordPreview: "Discord preview",
        previewUntitled: "Untitled event",
        previewNoDescription: "Your event description will appear here.",
    },
    presets: {
        topicTitle: "Topic presets",
        topicDescription:
            "Preset briefings are reference data. Events can copy them so later preset changes do not rewrite old matches.",
        squadTitle: "Squad presets",
        squadDescription:
            "These define the initial roster shape. New rosters copy from them so past events stay stable.",
        createTopicTitle: "Create topic preset",
        createTopicDescription: "Create and edit topic structure in one place.",
        createSquadTitle: "Create squad preset",
        createSquadDescription:
            "Create and reshape the preset structure in one place.",
        presetDetails: "Preset details",
        presetCreateMode: "Create mode starts directly in edit mode.",
        squadPresetPageDescription:
            "Preset squads and roles that can be copied into event rosters.",
        squadPresetMetaDescription: "Preset squad structure for new rosters.",
        topicPresetMetaDescription:
            "Reusable briefing templates copied into events when needed.",
        topics: "Topics",
        squadStructure: "Squad structure",
        presetSetup: "Preset setup",
        presetSetupDescription:
            "Preset information and squad structure live in one editor so the whole setup stays together.",
        addTopic: "Add topic",
        role: "Role",
        roleNote: "Role note",
        addRole: "Add role",
        squadBlock: "Squad block",
        presetName: "Preset name",
        topicEditorDescription:
            "Briefing topics are editable in the same admin edit flow as the preset details.",
        attachmentPlaceholder: "One attachment URL per line",
        attachmentCountSuffix: "attachment(s)",
        newTopic: "New topic",
        newSquad: "New squad",
        newRole: "New role",
        importStarterTemplate: "Import starter template",
        starterTemplateName: "HLL Standard Lineup",
        shortNote: "Note",
        untitledPreset: "Untitled preset",
        topicPresetMetaFallback: "Topic preset",
        topicPresetPageDescription:
            "Same structure as events, but focused on reusable briefing content.",
        squadPresetMetaFallback: "Squad preset",
        fields: {
            name: "Name",
            map: "Map",
            side: "Side",
            cap: "Cap",
            notes: "Notes",
        },
        table: {
            preset: "Preset",
            topics: "Topics",
            groups: "Groups",
            roleSlots: "Role slots",
        },
    },
    groups: {
        title: "Groups",
        description:
            "Groups are reusable clan specialties for players and squad presets. Rosters copy the names so older events stay intact.",
        createTitle: "Create group",
        createDescription:
            "Create a reusable clan specialty with a color and short description.",
        name: "Group name",
        color: "Color",
        descriptionLabel: "Description",
        order: "Order",
        parent: "Parent group",
        none: "None",
        reset: "Reset helper data",
        initialize: "Import default presets",
        resetConfirm:
            "This will delete groups, squad presets, and topic presets for this clan. Events, rosters, and players stay. Continue?",
        initializeConfirm:
            "This will reset groups, squad presets, and topic presets, then recreate the default group preset and starter squad preset. Continue?",
        resetSuccess: "Helper data reset complete.",
        initializeSuccess: "Default presets imported.",
        usedByPlayers: "Primary players",
        starterDescription:
            "Starter clan specialties aligned with the default HLL squad setup.",
    },
    tables: {
        event: "Event",
        meeting: "Meeting",
        status: "Status",
        pingClan: "Ping clan",
        enabled: "Enabled",
        disabled: "Disabled",
        squads: "Squads",
        reserves: "Reserves",
        visibility: "Visibility",
        hidden: "Hidden",
        unassigned: "Unassigned",
    },
    clan: {
        overviewMeta: "overview",
        adminAccess: "Admin access",
        memberAccess: "Member access",
        upcomingEvents: "Upcoming events",
        publishedRosters: "Published rosters",
        members: "Members",
        presets: "Presets",
        recentGames: "Recent games",
        lastTenGames: "Last 10 games",
        record: "Record",
        winRate: "Win rate",
        draw: "Draw",
        noRecentGames: "No recorded match results yet.",
        topPlayers: "Top players",
        matchesPlayed: "matches played",
        noPlayerStats: "No player statistics were imported for these matches.",
        performanceTrend: "Clan performance trend",
        playerPerformanceTrend: "Performance trend",
        performanceLastMatches: "Last {count} recorded matches",
        performanceComparedWithPrevious:
            "{change} {metric} vs previous {count}",
        performanceNoData:
            "No recorded match data yet. It will appear after the next stats import or refresh.",
        combat: "Combat",
        combatEffectiveness: "Combat effectiveness",
        offense: "Offense",
        support: "Support",
        supportEffectiveness: "Support effectiveness",
        deaths: "Deaths",
        points: "Points",
        kd: "K/D",
        kills: "Kills",
        topPlayersCalculation:
            "Players are ranked by their total kills across the last 10 games. Matches played breaks ties; K/D, offense, defense, and support are not included.",
        membersAndGroups: "Members and groups",
        joined: "Joined",
        snapshot: "Clan snapshot",
        visibilityTitle: "Who can see what",
        visibilityBody:
            "Calendar is visible to logged-in users in this clan. Events, presets, and unpublished rosters remain admin-only.",
        backendTitle: "Backend prep",
        backendBody:
            "Pages are wired around mock data and typed for Convex documents so we can drop in real queries later.",
        languageTitle: "Language",
        languageBody:
            "Locale routing is active with English dictionaries, so additional languages can be added without rewriting page structure.",
        importsTitle: "Imports",
        importsBody:
            "Bulk event, player, and platform identity imports live here so admin-only data tasks stay in one place.",
        helperDataTitle: "Helper data",
        helperDataBody:
            "Groups, squad presets, and topic presets are copy sources only. Resetting them never rewrites old events or rosters.",
        systemTitle: "System",
        systemBody: "Clan API keys and maintenance tools.",
        websiteApi: "Website API",
        webhooksTitle: "Webhooks",
        webhooksBody: "Configure signed notifications for clan changes.",
        webhookDelivery: "Webhook delivery",
        webhookDocumentation: "Read webhook documentation",
        webhookPayloadTitle: "Delivery payload and headers",
        webhookPayloadBody:
            "Logi sends a POST request. Verify the signature against the unmodified raw JSON bytes before parsing the body.",
        webhookHeadersBody:
            'The signature is HMAC-SHA256 of X-Logi-Timestamp + "." + the raw request body, using the webhook signing secret. X-Logi-Delivery identifies the delivery attempt and differs from the body id.',
        webhookApiReference: "API schema",
        webhookEventsTitle: "Events your endpoint can receive",
        webhookEventsBody:
            "New subscriptions receive every production event below. Use this list to prepare your endpoint before enabling a webhook.",
        webhookEvents: {
            "article.created":
                "An article was created through the Website API.",
            "article.updated":
                "An article was updated through the Website API.",
            "article.deleted":
                "An article was deleted through the Website API.",
            "event.created": "An event was created through the Website API.",
            "event.updated":
                "An event was updated or concluded through the Website API.",
            "roster.updated":
                "A roster changed, including API signups and roster-affecting assignment changes.",
            "settings.updated":
                "Safe clan or Discord settings were updated through the Website API.",
            "webhook.test": "A manager sent a test delivery manually.",
        },
        webhookUi: {
            copySecret:
                "Copy this signing secret now; it will not be shown again:",
            url: "Webhook URL",
            add: "Add webhook",
            loading: "Loading webhooks…",
            enabled: "Enabled",
            disabled: "Disabled",
            eventTypes: "event types",
            lastDelivery: "Last delivery",
            lastFailure: "Last failure",
            never: "never",
            enable: "Enable",
            disable: "Disable",
            sendTest: "Send test",
            rotate: "Rotate secret",
            history: "History",
            delete: "Delete",
            historyTitle: "Delivery history",
            noHistory: "No deliveries yet.",
            status: "Status",
            attempt: "Attempt",
            response: "Response",
            created: "Created",
            error: "Error",
        },
    },
    stratmaps: {
        title: "Stratmaps",
        createTitle: "Create Stratmap",
        pageDescription:
            "Saved realtime tactical maps for Hell Let Loose match planning.",
        createDescription:
            "Create a saved realtime tactical sketch that can later be attached to match briefings.",
        detailDescription:
            "Realtime tactical map collaboration for match planning.",
        viewMap: "View interactive stratmap and tactical planning.",
        searchPlaceholder: "Search stratmaps...",
        tableTitle: "Title",
        tableMap: "Map",
        tableSlides: "Slides",
        tableUpdated: "Updated",
        titleLabel: "Title",
        descriptionLabel: "Description",
        mapAndPoint: "Map and point",
        baseMap: "Map",
        searchMap: "Search map",
        point: "Point",
        searchPoint: "Search point",
        side: "Side",
        saveDetails: "Save details",
        detailsSaved: "Stratmap details saved.",
        saveDetailsError: "Unable to save stratmap details.",
        saveStateError: "Unable to save stratmap state.",
        createError: "Unable to create stratmap.",
        titleRequired: "Title is required.",
        importLabel: "Import from Maps Let Loose (optional)",
        importHint:
            "Upload the extracted mll_config.json from a Maps Let Loose export. Select the map details below before creating.",
        importInvalid: "This is not a valid Maps Let Loose JSON export.",
        importSummary: "{slides} slide(s) imported{skipped}.",
        liveAccess: "Admins can edit this stratmap. Others see updates live.",
        slides: "Slides",
        createSlideTitle: "Create slide",
        createSlideDescription:
            "Add a slide name and optionally upload a custom background image.",
        createSlideAction: "Create slide",
        slideNameLabel: "Slide name",
        customBackgroundLabel: "Custom background image",
        customBackgroundHint:
            "Leave empty to use the map background for this slide.",
        overlays: "Map overlays",
        board: "Board",
        toolProperties: "Tool properties",
        selectedIcon: "Selected icon",
        selectedElement: "Selected element",
        deleteSelected: "Delete selected",
        deleteTool: "Delete",
        undo: "Undo",
        redo: "Redo",
        oneSelected: "1 item selected",
        multipleSelected: "{count} items selected",
        quickColors: "Quick colors",
        stroke: "Stroke",
        fill: "Fill",
        strokeWidth: "Stroke width",
        lineStyle: "Line style",
        solid: "Solid",
        dashed: "Dashed",
        dotted: "Dotted",
        text: "Text",
        fontSize: "Font size",
        iconColor: "Icon color",
        iconLibrary: "Icon library",
        notes: "Notes",
        notePlaceholder: "Add context for this position.",
        images: "Images",
        imageDescription: "Image description",
        imageDescriptionPlaceholder: "Add context for this image.",
        mainImage: "Main image",
        attachImages: "Attach images",
        uploading: "Uploading...",
        noImages: "No images attached yet.",
        imagesAttached: "Images attached to icon.",
        uploadImagesError: "Unable to upload icon images.",
        selectedElementHint:
            "Use the select tool to move this element. Icon-specific notes and image attachments appear only when an icon is selected.",
        grid: "Grid",
        allStrongpoints: "All strongpoints",
        defaultGarrisons: "Default garrisons",
        artillery: "Artillery",
        repairStations: "Repair stations",
        spawnRanges: "Spawn ranges",
        overlaySide: "Overlay side",
        sideA: "Side A",
        sideB: "Side B",
        visibleStrongpoints: "Visible strongpoints",
        selectTool: "Select",
        drawTool: "Draw",
        lineTool: "Line",
        polygonTool: "Polygon",
        measureTool: "Measure",
        rectTool: "Rect",
        circleTool: "Circle",
        pingTool: "Ping",
        iconLabel: "Icon",
        zoomIn: "Zoom in",
        zoomOut: "Zoom out",
        resetZoom: "Reset zoom",
        lineStart: "Line start",
        lineEnd: "Line end",
        showDistance: "Show distance",
        none: "None",
        arrow: "Arrow",
        circleMarker: "Circle",
        squareMarker: "Square",
        color: "Color",
        size: "Size",
        noResults: "No results.",
    },
    onboarding: {
        step: "Step {current} of {total}",
        back: "Back",
        next: "Next",
        finish: "Finish",
        skip: "Skip tour",
        saveError: "We could not save your tour progress. Please try again.",
        setupWelcome: {
            title: "Welcome to your command center",
            description:
                "This short setup guide appears once. Choose a workspace first, then Logi will show the menus that apply to you.",
        },
        setupWorkspace: {
            title: "Choose a workspace",
            description:
                "Use the workspace switcher in the left sidebar to open the community you belong to or manage. Each workspace has separate events, members, and Discord settings.",
        },
        setupBot: {
            title: "Invite the Discord bot",
            description:
                "If you manage a workspace and the bot is not inside yet, use the Invite bot button on this dashboard. Return here after Discord confirms the invite and refresh the bot status.",
        },
        setupBotNote:
            "The bot needs the permissions required by the features you enable. Place its Discord role above every role it must manage.",
        setupNext: {
            title: "Open your workspace tour",
            description:
                "After you select a workspace, Logi will show a short tour tailored to your access: a member tour or a manager tour.",
        },
        memberWelcome: {
            title: "This is your workspace",
            description:
                "The sidebar now shows the areas available in this community. Use the workspace switcher any time you need to move to another community.",
        },
        memberOperations: {
            title: "Follow operations",
            description:
                "Events, matches, trainings, rosters, and briefings are where you check schedules, sign up, see your assignment, and review plans shared by your managers.",
        },
        memberAccount: {
            title: "Keep your account ready",
            description:
                "Open your account menu to update your avatar and platform IDs. A platform ID may be required before you can submit a membership application.",
        },
        managerWelcome: {
            title: "You can manage this workspace",
            description:
                "Your sidebar includes the operational and configuration areas for this community. Changes here affect this workspace only.",
        },
        managerOperations: {
            title: "Plan and publish operations",
            description:
                "Create events, matches, trainings, rosters, topic presets, and stratmaps from Operations. Draft first, then publish only after you have checked the Discord configuration and audience.",
        },
        managerConfiguration: {
            title: "Configure before automating",
            description:
                "Members, membership applications, groups, tickets, and Server settings control what the bot can do. Configure channels and roles before enabling Discord-dependent workflows.",
        },
    },
    shared: {
        openColumn: "Open",
        nothingCreatedYet: "Nothing has been created in this section yet.",
        notSet: "-",
        detectedPlatformId: "Detected: {platform}",
        platformSteam: "Steam",
        platformEpic: "Epic Games",
        platformXbox: "Xbox",
        platformPlayStation: "PlayStation",
        platformOther: "Xbox / PlayStation / Other",
        createMode: "Create mode starts directly in edit mode.",
        searchTable: "Search this table...",
        noMatchingResults: "No matching results found.",
        previousPage: "Previous",
        nextPage: "Next",
        pageLabel: "Page {page} of {pages}",
        showingResults: "Showing {from}-{to} of {total}",
        rosterMetaDescription:
            "Roster board with reserves, role slots, publish state, and acknowledgements.",
        rosterPageDescription:
            "Inspired by competitive roster boards: grouped squads, visible reserves, assignment status, and a future-ready acknowledgement flow.",
    },
} as const

export type AppMessages = typeof enMessages
