...
export class ConvexAssignmentCommandRepository implements AssignmentCommandRepository {
        ...
        const server =
            (await this.ctx.db
                ...
                .withIndex("discordId", (q) =>
                    q.eq("discordId", serverDiscordId)
                )
                .unique()) ??
            (await this.ctx.db
                .query("guilds")
                .withIndex("id", (q) => q.eq("id", serverDiscordId))
                .unique())
    ...
    async save(input: {
        ...
        const payload = {
            ...
            primaryGroupId: input.primaryGroupId
                ? (input.primaryGroupId as Id<"groups">)
                : undefined,
            secondaryGroupIds: input.secondaryGroupIds.map(
    ...
    async upsertImportedUser(input: {
        ...
        const existingUser =
            (await this.ctx.db
                .query("users")
                .withIndex("discordId", (q) => q.eq("discordId", input.userId))
                .unique()) ??
            (await this.ctx.db
                .query("users")
                .withIndex("id", (q) => q.eq("id", input.userId))
                .unique())
        const avatar =
            input.avatar ||
            existingUser?.avatar ||
        ...
        const nicknames = {
            ...
            ...(input.serverDiscordId && input.nickname
                ? { [input.serverDiscordId]: input.nickname }
                : {}),
        }
    ...
    private normalizeAssignment(assignment: any): AssignmentRecord {
        return {
            ...
            primaryGroupId: assignment.primaryGroupId
                ...
                : undefined,
            secondaryGroupIds: Array.isArray(assignment.secondaryGroupIds)
                ? assignment.secondaryGroupIds.map((groupId: unknown) =>
                      String(groupId)
                  )
                : [],
        }
