...
export class InMemoryAssignmentCommandRepository implements AssignmentCommandRepository {
    constructor(
        ...
        public readonly serverMembershipPatches: Array<{
            serverDiscordId: string
            patch: any
        }> = [],
        public readonly userMembershipPatches: Array<{
            userId: string
            patch: any
        }> = [],
        public readonly importedUsers: Map<
            string,
            {
                name: string
                avatar: string
    ...
    async listByServer(serverDiscordId: string): Promise<AssignmentRecord[]> {
        return [...this.assignments.values()].filter(
            (item) => item.serverId === serverDiscordId
        )
    }
    ...
    async save(input: {
        ...
        const id =
            input.assignmentId ?? `assignment-${this.assignments.size + 1}`
