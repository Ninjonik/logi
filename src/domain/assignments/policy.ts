...
export function validateAssignmentGroupIds(input: {
    ...
    if (
        ...
        !input.validGroupIds.has(String(input.primaryGroupId))
    ) {
        throw new Error("Primary group does not belong to this server.")
    }

    if (
        input.secondaryGroupIds.some(
            (groupId) => !input.validGroupIds.has(String(groupId))
        )
    ) {
        throw new Error(
            "One of the selected secondary groups does not belong to this server."
        )
    }
}
...
export function mergeImportedSecondaryGroupIds(input: {
    ...
    return [
        ...new Set([
            ...input.existingSecondaryGroupIds.map((groupId) =>
                String(groupId)
            ),
            ...input.importedSecondaryGroupIds.map((groupId) =>
                String(groupId)
            ),
        ]),
