...
export function buildServerMembershipState(input: {
    ...
    const activeMemberAssignments = input.assignments.filter(
        (item) =>
            (item.type === "member" || item.type === "reserve_member") &&
            item.status !== "pending"
    )
    const activeMercAssignments = input.assignments.filter(
        (item) => item.type === "mercenary" && item.status === "active"
    )
    ...
    return {
        ...
        members: activeMemberAssignments.map((item) => ({
            id: item.userId,
            primaryGroup: item.primaryGroupId
                ? input.groupNameById.get(String(item.primaryGroupId))
                : undefined,
...
export function buildUserMembershipState(input: {
    ...
    const primaryAssignment = input.assignments.find(
        (item) =>
            (item.type === "member" || item.type === "reserve_member") &&
            item.status !== "pending"
    )
