...
export function filterCollection<T extends RecordValue>(items: readonly T[], filters: readonly CollectionFilter[]) {
  return items.filter((item) => filters.every(({ path, value }) => {
    const [first, second] = path.split(".");
    const topLevelValue = item[first];
    const candidate = second && topLevelValue && typeof topLevelValue === "object" && !Array.isArray(topLevelValue)
...
export function paginateCollection<T>(items: readonly T[], offset: number, limit: number) {
  const page = items.slice(offset, offset + limit);
  const nextOffset = offset + page.length;
  return { page, total: items.length, offset, limit, nextOffset: nextOffset < items.length ? nextOffset : null };
}
