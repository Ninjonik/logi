...
} from "@/domain/shared/collections"
...
function syncReserveAttendances(
    ...
    const nextAttendances = previousAttendances.filter((entry) =>
        reserveUserIdSet.has(entry.userId)
    )

    if (
        ensureUserId &&
        reserveUserIdSet.has(ensureUserId) &&
        !nextAttendances.some((entry) => entry.userId === ensureUserId)
    ) {
...
function moveUserToBucket(
    ...
    roster.reservePlayerIds = roster.reservePlayerIds.filter(
        (id) => id !== userId
    )
    roster.notAttendingPlayerIds = roster.notAttendingPlayerIds.filter(
        (id) => id !== userId
    )

    if (bucket === "reserve") {
        roster.reservePlayerIds = dedupePreservingOrder([
    ...
    if (bucket === "not_attending") {
        roster.notAttendingPlayerIds = dedupePreservingOrder([
...
function buildTrackedUserIds(
    event: EventRecord,
    assignments: AssignmentRecord[],
    now: Date
) {
    const participantNow =
        event.updatedAt ?? event.createdAt ?? now.toISOString()
...
export function mergeRosterWithEventState<T extends RosterLike>(
    ...
    const preservedUserIds = options?.preservePlacedUsers
        ? getRosteredUserIds(next)
        : new Set<string>()
