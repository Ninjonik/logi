...
export function canAcceptSignups(
    ...
    return (
        ...
        isTrainingRegistrationStillOpen(event, now)
    )
}
...
export function isEventCancelledBeforeMeeting(event: {
    ...
    return (
        Number.isFinite(concludedAt) &&
        Number.isFinite(meetingStart) &&
        concludedAt < meetingStart
    )
}

export function normalizeEventTimestamps<T extends EventLike>(
    event: T,
    nowIso: string
) {
    return {
