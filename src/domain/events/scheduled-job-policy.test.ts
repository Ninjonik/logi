...
import { getAttendanceReminderDueAt, isExpiredScheduledJobClaim, shouldDiscardScheduledJob } from "./scheduled-job-policy";
...
test("past later reminder windows are not backfilled and past meetings receive no reminder", () => {
  assert.equal(getAttendanceReminderDueAt("2026-08-29T22:00:00.000Z", 18, now), null);
  assert.equal(getAttendanceReminderDueAt("2026-08-29T16:00:00.000Z", 24, now), null);
});
