...
const getServerContextReference = makeFunctionReference<"query">(
    "serverData:getServerContext"
)
const syncDiscordProfileReference = makeFunctionReference<"mutation">(
    "players:syncDiscordProfile"
)
const upsertAssignmentReference = makeFunctionReference<"mutation">(
    "userAssignments:upsert"
)
const upsertEventReference = makeFunctionReference<"mutation">("events:upsert")
const upsertRosterReference =
    ...
    return [
        {
            name: "VLK vs BxB",
            description:
                "Friendly match imported from HeLO match history for Valkyria test data.",
            map: "Omaha Beach",
            side: "Axis",
            cap: "Defeat (2-3)",
            date: "2026-06-28",
            durationMinutes: 90,
            matchUrl:
                "https://helo-system.de/statistics/matches/BxB-VLK-2026-06-28?series=2024",
            opponent: "BxB",
        },
        {
            name: "VLK vs LCM",
            ...
            durationMinutes: 72,
            matchUrl:
                "https://helo-system.de/statistics/matches/VLK-LCM-2026-05-24?series=2024",
            opponent: "LCM",
        },
        {
            name: "VLK vs HTD",
            description:
                "Competitive ECL match imported from HeLO match history for Valkyria test data.",
            map: "SMDM",
            side: "Axis",
            cap: "Defeat (2-3)",
            date: "2026-04-26",
            durationMinutes: 90,
            matchUrl:
                "https://helo-system.de/statistics/matches/HTD-VLK-2026-04-26?series=2024",
            competition: "ECL",
            opponent: "HTD",
        },
        {
            ...
            durationMinutes: 90,
            matchUrl:
                "https://helo-system.de/statistics/matches/VLK-WAR-2026-04-19?series=2024",
            opponent: "LORD",
        },
        {
            name: "VLK vs FCo",
            ...
            durationMinutes: 90,
            matchUrl:
                "https://helo-system.de/statistics/matches/VLK-FF-2026-04-12?series=2024",
            competition: "ECL",
            opponent: "FCo",
        },
        {
            ...
            durationMinutes: 90,
            matchUrl:
                "https://helo-system.de/statistics/matches/VLK-PZJR-2026-03-22?series=2024",
            opponent: "PZJR",
        },
        {
            name: "VLK vs FLL",
            ...
            durationMinutes: 90,
            matchUrl:
                "https://helo-system.de/statistics/matches/VLK-FLL-2026-03-15?series=2024",
            competition: "ECL",
            opponent: "FLL",
        },
        {
            ...
            durationMinutes: 90,
            matchUrl:
                "https://helo-system.de/statistics/matches/HTD-VLK-2026-03-01?series=2024",
            competition: "ECL",
            opponent: "HTD",
        },
    ]
...
async function main() {
    ...
    if (!convexUrl) {
        throw new Error(
            "Missing Convex URL env. Expected NEXT_PUBLIC_CONVEX_URL or CONVEX_SELF_HOSTED_URL."
        )
    }
    ...
    if (!adminUserId) {
        throw new Error(
            `Guild ${guildId} has no admin user, so server context cannot be loaded.`
        )
    }
    ...
    const groupIdByName = new Map(
        initialContext.groups.map((group) => [group.name, group.id])
    )
    ...
    for (const groupName of requiredGroups) {
        if (!groupIdByName.has(groupName)) {
            throw new Error(
                `Group "${groupName}" is missing in guild ${guildId}. Initialize helper data first.`
            )
        }
    }

    const existingAssignmentByUserId = new Map(
        initialContext.assignments.map((assignment) => [
            assignment.userId,
            assignment,
        ])
    )
    const existingEventByName = new Map(
        initialContext.events.map((event) => [event.name, event])
    )
    const existingRosterByEventId = new Map(
        initialContext.rosters.map((roster) => [roster.eventId, roster])
    )
    const defaultPresetId = initialContext.squadPresets.find(
        (preset) => preset.name === "HLL Standard Lineup"
    )?.id

    const seedPlayers = buildSeedPlayers()
    console.log(
        `Seeding ${seedPlayers.length} mock VLK players into guild ${guildId}...`
    )
    ...
    if (!refreshedContext) {
        throw new Error(
            `Unable to reload server context for guild ${guildId} after seeding.`
        )
    }

    const rosterPlayers = seedPlayers
    const rosterPayload = buildRosterForPlayers(rosterPlayers)
    const refreshedEventByName = new Map(
        refreshedContext.events.map((event) => [event.name, event])
    )
    const refreshedRosterByEventId = new Map(
        refreshedContext.rosters.map((roster) => [roster.eventId, roster])
    )
    ...
    for (const event of buildSeedEvents()) {
        ...
        const existingRoster =
            refreshedRosterByEventId.get(currentEvent.id) ??
            existingRosterByEventId.get(currentEvent.id)
    ...
    console.log(
        `Created or updated ${seededEventIds.length} events with published rosters.`
    )
}
