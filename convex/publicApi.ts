...
function websiteClan(guild: Parameters<typeof normalizeGuildDoc>[0]) {
    ...
    const safe = { ...normalized } as Record<string, unknown>
    for (const field of ["adminIds", "memberIds", "mercenaryIds", "members"])
        delete safe[field]
    return safe
}
...
export const getClanData = query({
    ...
    handler: async (ctx, args) => {
        ...
        return {
            clan: websiteClan(guild),
            events: events.map(websiteEvent),
            groups: groups.map(normalizeDoc),
            rosters: rosters.map(normalizeDoc),
            assignments: assignments.map((assignment) =>
                normalizeAssignmentDoc(assignment, groupNames)
            ),
            ...
            articles: articles.map(normalizeDoc),
            matches: matchStats.map((match) => ({
                ...match,
                id: String(match._id),
