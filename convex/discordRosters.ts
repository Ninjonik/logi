...
export const getRosterImageContext = query({
    ...
    handler: async (ctx, args) => {
        ...
        const users = usersRaw.filter(
            (user): user is NonNullable<(typeof usersRaw)[number]> =>
                Boolean(user)
        )
        return {
            ...
            users: users.map((user) =>
                normalizeUserDoc(user, { guildId: event.guildId })
            ),
        }
    },
})
...
export const confirmRosterAttendanceFromMeetingChannel = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const memberIdsInMeetingChannel = new Set(
            args.memberIdsInMeetingChannel
        )
        ...
        const reserveAttendances = (roster.reserveAttendances ?? []).map(
            (entry) => {
        ...
        for (const userId of roster.reservePlayerIds) {
            if (
                !memberIdsInMeetingChannel.has(userId) ||
                reserveAttendances.some((entry) => entry.userId === userId)
            )
                continue
            reserveCount += 1
            updatedCount += 1
