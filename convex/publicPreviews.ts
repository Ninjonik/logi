...
export const ensure = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .unique()
        if (existing) {
            await ctx.db.patch(existing._id, { expiresAt, updatedAt: now })
            return { ...existing, expiresAt, updatedAt: now }
        }
        ...
        if (args.entityType === "match") {
            ...
            const match = event
                ? await ctx.db
                      ...
                      .unique()
                : null
            if (!event || !match || event.matchStatsId !== match._id)
                return null
            const score = `${match.raw.result.allied} – ${match.raw.result.axis}`
            title = `${event.name} · ${score}`
            description = `${match.raw.map.pretty_name} · Recorded match result`
        } else if (args.entityType === "player") {
            const user =
                (await ctx.db
                    .query("users")
                    .withIndex("id", (q) => q.eq("id", args.entityId))
                    .unique()) ??
                (await ctx.db
                    .query("users")
                    .withIndex("discordId", (q) =>
                        q.eq("discordId", args.entityId)
                    )
                    .unique())
            if (!user) return null
            const stats = await ctx.db
                .query("playerStats")
                .withIndex("userId", (q) => q.eq("userId", args.entityId))
                .collect()
            const matches = stats.flatMap((entry) =>
                Object.values(entry.matches)
            )
            if (!matches.length) return null
            const kills = matches.reduce((sum, match) => sum + match.kills, 0)
            const deaths = matches.reduce((sum, match) => sum + match.deaths, 0)
            title = user.name
            description = `${matches.length} recorded matches · ${(deaths ? kills / deaths : kills).toFixed(2)} K/D`
        } else {
            const guild =
                (await ctx.db
                    .query("guilds")
            ...
            title = guild.name
            description = "Public clan profile and recorded match history."
        }
        const preview = {
