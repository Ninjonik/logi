...
export async function rebuildGuildPerformanceHistory(
  ...
  const userByPlatformId = new Map(
    playerDocs.filter((doc) => doc.userId).map((doc) => [doc.id, doc.userId!])
  );
  ...
  for (let index = 0; index < guildEvents.length; index++) {
    ...
    guildMatches.push({
      ...
      kd:
        whole((total.deaths ? total.kills / total.deaths : total.kills) * 100) /
        100,
    });
  }
  ...
  return (
    history && {
      ...
      matches: history.matches.map((match) => ({
        ...
        kd:
          match.kd ?? (match.deaths ? match.kills / match.deaths : match.kills),
      })),
    }
  );
}
  ...
  handler: async (ctx, args) => {
    ...
    return Object.fromEntries(
      rows
        ...
        .map((row) => [
          row.userId,
          filterByGameScope(normalizeHistory(row)!.matches, args.gameScope),
        ])
    );
  },
});
