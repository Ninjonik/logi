...
export function getGuildDiscordId(
    guild: Pick<LegacyGuild, "discordId"> & { id?: string }
) {
    return guild.discordId ?? guild.id ?? ""
}

export function getUserDiscordId(
    user: Pick<LegacyUser, "discordId"> & { id?: string }
) {
    return user.discordId ?? user.id ?? ""
}

export function getUserStableId(
    user: Pick<LegacyUser, "discordId"> & { id?: string; _id?: unknown }
) {
...
export async function getGuildByDiscordId(ctx: DbCtx, discordId: string) {
    return (
        ...
        (await ctx.db
            .query("guilds")
            .withIndex("id", (q) => q.eq("id", discordId))
            .unique())
    )
}
...
export async function getUserByIdentifier(ctx: DbCtx, identifier: string) {
    return (
        ...
        (await ctx.db
            .query("users")
            .withIndex("discordId", (q) => q.eq("discordId", identifier))
            .unique())
    )
}
