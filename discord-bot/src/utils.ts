...
export function getRosterImageVersion(
    event: EventRecord,
...
export function buildForumThreadName(
    config: DiscordConfig,
    event: EventRecord
) {
    return `${event.name} ${formatShortDate(event.gameStart, config.timezone, config.defaultLanguage)}`.slice(
        0,
        100
    )
}
