...
test("resolveMembershipRoleIds maps statuses to configured roles", () => {
    ...
    assert.deepEqual(
        resolveMembershipRoleIds(config, "member", "pending", "infantry"),
        []
    )
    assert.deepEqual(
        resolveMembershipRoleIds(config, "member", "recruit", "infantry"),
        ["clan", "recruit", "recruit-2"]
    )
    assert.deepEqual(
        resolveMembershipRoleIds(config, "member", "active", "infantry"),
        ["clan", "member", "member-2"]
    )
})
