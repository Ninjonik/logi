...
export async function syncTicketPanel(client: Client, payload: SyncPayload) {
    ...
    if (
        ...
        logInfo(
            "ticket-panel",
            "Skipping ticket panel sync because configuration is incomplete",
            {
    ...
    if (!guild) {
        logWarn(
            "ticket-panel",
            "Skipping ticket panel sync because guild could not be fetched",
            {
                guildId: payload.config.guildId,
            }
        )
    ...
    const channel = await guild.channels
        .fetch(ticketSettings.submitChannelId)
        .catch(() => null)
    if (!channel?.isTextBased() || channel.type !== ChannelType.GuildText) {
        logWarn(
            "ticket-panel",
            "Skipping ticket panel sync because submit channel is unavailable or not text",
            {
                guildId: payload.config.guildId,
    ...
    const currentMessage = payload.config.ticketPanelMessageId
        ? await textChannel.messages
              .fetch(payload.config.ticketPanelMessageId)
              .catch(() => null)
        : null
...
export async function syncMembershipPanel(
    ...
    if (
        ...
        !membershipSettings.categories.length
    ) {
        logInfo(
            "membership-panel",
            "Skipping membership panel sync because configuration is incomplete",
            {
                guildId: payload.config.guildId,
    ...
    if (!guild) {
        logWarn(
            "membership-panel",
            "Skipping membership panel sync because guild could not be fetched",
            {
                guildId: payload.config.guildId,
            }
        )
    ...
    const channel = await guild.channels
        .fetch(membershipSettings.submitChannelId)
        .catch(() => null)
    if (!channel?.isTextBased() || channel.type !== ChannelType.GuildText) {
        logWarn(
            "membership-panel",
            "Skipping membership panel sync because submit channel is unavailable or not text",
            {
                guildId: payload.config.guildId,
    ...
    if (currentMessage) {
        logInfo(
            "membership-panel",
            "Updating existing membership panel message",
            {
                guildId: payload.config.guildId,
                messageId: currentMessage.id,
            }
    ...
    if (
        ...
        payload.config.membershipPanelLastConfigUpdatedAt !==
            payload.config.updatedAt
    ) {
...
export async function syncCalendarPanel(client: Client, payload: SyncPayload) {
    ...
    if (!guild) {
        logWarn(
            "calendar-panel",
            "Skipping calendar panel sync because guild could not be fetched",
            {
                guildId: payload.config.guildId,
            }
        )
    ...
    if (!payload.config.calendarChannelId) {
        if (storedChannelId && storedMessageId) {
            const previousChannel = await guild.channels
                .fetch(storedChannelId)
                .catch(() => null)
            if (
                previousChannel?.isTextBased() &&
        ...
        logInfo(
            "calendar-panel",
            "Skipping calendar panel sync because calendar channel is not configured",
            {
                guildId: payload.config.guildId,
            }
        )
    ...
    const channel = await guild.channels
        .fetch(payload.config.calendarChannelId)
        .catch(() => null)
    if (!channel?.isTextBased() || channel.type !== ChannelType.GuildText) {
        logWarn(
            "calendar-panel",
            "Skipping calendar panel sync because calendar channel is unavailable or not text",
            {
                guildId: payload.config.guildId,
    ...
    if (
        storedChannelId &&
        storedChannelId !== textChannel.id &&
        storedMessageId
    ) {
        const previousChannel = await guild.channels
            .fetch(storedChannelId)
    ...
    const currentMessage =
        storedMessageId && storedChannelId === textChannel.id
            ? await textChannel.messages
                  .fetch(storedMessageId)
                  .catch(() => null)
            : null
    ...
    if (
        ...
        payload.config.calendarMessageLastConfigUpdatedAt !==
            payload.config.updatedAt
    ) {
        await convex.mutation(references.updateCalendarPanelState, {
            secret: env.internalSecret,
