...
import {
    getCalendarSyncVersion,
    shouldRefreshCalendar,
    shouldSyncEventRoles,
} from "./work"
...
test("displayed schedule changes refresh calendar and roster changes refresh event roles", () => {
    ...
    assert.equal(
        shouldRefreshCalendar(before, { ...before, name: "Renamed" }),
        true
    )
    assert.equal(
        shouldRefreshCalendar(before, {
            ...before,
            gameStart: "2099-01-01T20:30:00.000Z",
        }),
        true
    )
    assert.equal(
        shouldSyncEventRoles(before, { ...before, status: "concluded" }),
        true
    )
    assert.notEqual(
        getCalendarSyncVersion(before),
        getCalendarSyncVersion({ ...before, name: "Renamed" })
    )
})
