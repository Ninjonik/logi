...
test("getAnnouncementPingRoleIds resolves and de-duplicates selected roles", () => {
  const event = { pingClan: false, pingMode: "roles", pingRoleIds: ["role-1", "role-1", " role-2 ", ""] } as EventRecord;

  assert.deepEqual(getAnnouncementPingRoleIds(payload, event), ["role-1", "role-2"]);
});
